...
DOCUMENTATION = """
---
module: fortios_wireless_controller_wag_profile
short_description: Configure wireless access gateway (WAG) profiles used for tunnels on AP in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify wireless_controller feature and wag_profile category.
...
def filter_wireless_controller_wag_profile_data(json):
    option_list = [
        "comment",
        "dhcp_ip_addr",
        "name",
...
def wireless_controller_wag_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "wag-profile", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_wag_profile"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_wag_profile"]:
        resp = wireless_controller_wag_profile(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_wag_profile"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_wag_profile"
        )
