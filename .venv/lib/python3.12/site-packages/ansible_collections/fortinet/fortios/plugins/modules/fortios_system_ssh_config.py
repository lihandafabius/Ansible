...
def system_ssh_config(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "ssh-config", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "ssh-config", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ssh_config"] = filtered_data
    ...
    return fos.set(
        "system", "ssh-config", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_ssh_config"]:
        resp = system_ssh_config(data, fos, check_mode)
    else:
