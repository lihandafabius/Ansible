...
class HttpApi(HttpApiBase):
    ...
    def login(self, username, password):
        ...
        if self.get_access_token() is not None:
            ...
            # /logincheck returns (gzipped) HTML, and Ansible's httpapi stack may
            # attempt to treat the body as gzip-compressed JSON, causing:
            #   BadGzipFile("Not a gzipped file ...")
            # Validate the token using a JSON API endpoint instead.
        ...
        # Cache password for features that need per-request confirmation headers.
        ...
        self.log('API based auth returned status code %s, data: %s' % (status_code, result_data))
        if status_code in [401, 404]:
            ...
            try:
                ...
                self.log('no session_key obtained from response body, fallback to parse headers, data %s' % (result_data))
                # pass
    ...
    def update_auth(self, response, response_text):
        ...
        if self._cookie or self._ccsrf_token:
            self.log('using existing cookie: %s; ccsrf_token %s' % (self._cookie, self._ccsrf_token))
        ...
        for attr, val in response.getheaders():
            if attr.lower() == 'set-cookie' and 'APSCOOKIE_' in val:
            ...
            elif attr.lower() == 'set-cookie' and ('ccsrftoken' in val or 'ccsrf_token' in val):
                csrftoken_search = re.search('(.*)', val)

                self.log(
                    'found csrftoken returned from server: %s, filtered %s'
                    % (
                        to_text(val),
                ...
                self.log('other raw cookie returned from server: %s' % to_text(val.split(';')[0]))
                raw_cookies.append(val.split(';')[0])
    ...
    def send_request(self, **message_kwargs):
        ...
        # Some APIs require X-Admin-Passwd header (since FortiOS 7.6.5 for a few admin-related APIs).
        # Generator-enabled modules may pass a placeholder True; inject the real password here.
        if "X-Admin-Passwd" in headers and headers.get("X-Admin-Passwd") is True:
            if self._admin_password is None:
                # Best-effort fallback; may be unavailable depending on Ansible version/connection.
                try:
                    self._admin_password = self._conn.get_option("password")
                except Exception:
                    self._admin_password = None
        ...
        if "X-Admin-Passwd" in headers:
            safe_headers = dict(headers)
            safe_headers["X-Admin-Passwd"] = "***"
        self.log(
            "Sending request: METHOD:%s URL:%s DATA:%s HEADERS:%s"
            % (method, url, data, safe_headers)
        )
        ...
        try:
            ...
            self.log("response status: %s, data: %s...<truncated>" % (to_text(response.status), json_formatted[:200]))
