...
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify emailfilter feature and bwl category.
...
def filter_emailfilter_bwl_data(json):
...
def emailfilter_bwl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("emailfilter", "bwl", filtered_data, vdom=vdom)
        current_data = fos.get(
            "emailfilter", "bwl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["emailfilter_bwl"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "emailfilter", "bwl", data=converted_data, vdom=vdom, parameters=parameters
        )
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "id": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "integer", "required": True},
        "name": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "string"},
        "comment": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "string"},
        "entries": {
            "type": "list",
            "elements": "dict",
            "children": {
                ...
                "ip4_subnet": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "string"},
                "ip6_subnet": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "string"},
                "pattern_type": {
                    "v_range": [["v6.2.0", "v6.4.4"]],
                    "type": "string",
                    "options": [{"value": "wildcard"}, {"value": "regexp"}],
                },
                "email_pattern": {"v_range": [["v6.2.0", "v6.4.4"]], "type": "string"},
            },
            "v_range": [["v6.2.0", "v6.4.4"]],
        },
    },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["emailfilter_bwl"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["emailfilter_bwl"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "emailfilter_bwl"
        )
