...
def router_bfd6(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "bfd6", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "bfd6", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_bfd6"] = filtered_data
    ...
    return fos.set(
        "router", "bfd6", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_bfd6"]:
        resp = router_bfd6(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("router_bfd6"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_bfd6"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["router_bfd6"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_bfd6"
        )
