...
options:
    ...
    waf_main_class:
        ...
        suboptions:
            id:
                description:
                    - Main signature class ID. see <a href='#notes'>Notes</a>.
                required: true
                type: int
            name:
                description:
...
def waf_main_class(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("waf", "main-class", filtered_data, vdom=vdom)
        current_data = fos.get(
            "waf", "main-class", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["waf_main_class"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "waf", "main-class", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_waf(data, fos, check_mode):
    ...
    if data["waf_main_class"]:
        resp = waf_main_class(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("waf_main_class"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["waf_main_class"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["waf_main_class"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "waf_main_class"
        )
