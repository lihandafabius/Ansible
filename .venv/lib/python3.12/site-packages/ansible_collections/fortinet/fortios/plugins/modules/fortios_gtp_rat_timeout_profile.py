...
def filter_gtp_rat_timeout_profile_data(json):
    option_list = [
        "eutran_timeout",
        "gan_timeout",
        "geran_timeout",
...
def gtp_rat_timeout_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("gtp", "rat-timeout-profile", filtered_data, vdom=vdom)
        current_data = fos.get(
            "gtp", "rat-timeout-profile", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["gtp_rat_timeout_profile"] = filtered_data
...
def fortios_gtp(data, fos, check_mode):
    ...
    if data["gtp_rat_timeout_profile"]:
        resp = gtp_rat_timeout_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("gtp_rat_timeout_profile"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["gtp_rat_timeout_profile"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["gtp_rat_timeout_profile"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "gtp_rat_timeout_profile"
        )
