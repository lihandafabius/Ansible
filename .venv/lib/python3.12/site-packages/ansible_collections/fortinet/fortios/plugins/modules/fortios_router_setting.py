...
def router_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_setting"] = filtered_data
    ...
    return fos.set(
        "router", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_setting"]:
        resp = router_setting(data, fos, check_mode)
    else:
