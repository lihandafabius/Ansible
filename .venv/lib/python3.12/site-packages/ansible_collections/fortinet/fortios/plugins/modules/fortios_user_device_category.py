...
def user_device_category(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("user", "device-category", filtered_data, vdom=vdom)
        current_data = fos.get(
            "user", "device-category", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["user_device_category"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["user_device_category"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["user_device_category"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "user_device_category"
        )
