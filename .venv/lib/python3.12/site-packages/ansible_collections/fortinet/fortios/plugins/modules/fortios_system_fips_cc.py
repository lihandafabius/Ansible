...
def system_fips_cc(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "fips-cc", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "fips-cc", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_fips_cc"] = filtered_data
    ...
    return fos.set(
        "system", "fips-cc", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_fips_cc"]:
        resp = system_fips_cc(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_fips_cc"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "self_test_period": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "key_generation_self_test": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_fips_cc"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_fips_cc"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_fips_cc"
        )
