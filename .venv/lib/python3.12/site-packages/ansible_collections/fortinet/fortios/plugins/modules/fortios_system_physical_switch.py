...
def filter_system_physical_switch_data(json):
    option_list = ["age_enable", "age_val", "name", "port"]
...
def system_physical_switch(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "physical-switch", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "physical-switch", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_physical_switch"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_physical_switch"]:
        resp = system_physical_switch(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_physical_switch"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_physical_switch"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_physical_switch"
        )
