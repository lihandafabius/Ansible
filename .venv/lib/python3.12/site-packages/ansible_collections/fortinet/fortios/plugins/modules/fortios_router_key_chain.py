...
def router_key_chain(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "key-chain", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "key-chain", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_key_chain"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "router", "key-chain", data=converted_data, vdom=vdom, parameters=parameters
        )
    ...
    "children": {
        ...
        "key": {
            ...
            "children": {
                ...
                "accept_lifetime": {"v_range": [["v6.0.0", ""]], "type": "string"},
                "send_lifetime": {"v_range": [["v6.0.0", ""]], "type": "string"},
                "key_string": {"v_range": [["v6.0.0", ""]], "type": "string"},
                "algorithm": {
                    "v_range": [["v7.0.1", ""]],
                    "type": "string",
                    "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_key_chain"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_key_chain"
        )
