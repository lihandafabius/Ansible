...
def firewall_acl6(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_acl6"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_acl6"]:
        resp = firewall_acl6(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_acl6"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_acl6"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_acl6"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_acl6"
        )
