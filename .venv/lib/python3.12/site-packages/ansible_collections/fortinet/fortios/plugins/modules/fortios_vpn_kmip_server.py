...
def vpn_kmip_server(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn", "kmip-server", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn", "kmip-server", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_kmip_server"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "vpn", "kmip-server", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_vpn(data, fos, check_mode):
    ...
    if data["vpn_kmip_server"]:
        resp = vpn_kmip_server(data, fos, check_mode)
    else:
