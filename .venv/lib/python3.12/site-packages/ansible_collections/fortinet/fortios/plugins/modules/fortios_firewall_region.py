...
short_description: Define region table in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify firewall feature and region category.
...
def firewall_region(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_region"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_region"]:
        resp = firewall_region(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_region"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_region"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_region"
        )
