...
def system_replacemsg_sslvpn(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system.replacemsg", "sslvpn", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system.replacemsg", "sslvpn", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_replacemsg_sslvpn"] = filtered_data
...
def fortios_system_replacemsg(data, fos, check_mode):
    ...
    if data["system_replacemsg_sslvpn"]:
        resp = system_replacemsg_sslvpn(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_replacemsg_sslvpn")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_replacemsg_sslvpn"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_sslvpn"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_replacemsg_sslvpn"
        )
