...
def videofilter_keyword(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("videofilter", "keyword", filtered_data, vdom=vdom)
        current_data = fos.get(
            "videofilter", "keyword", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["videofilter_keyword"] = filtered_data
...
def fortios_videofilter(data, fos, check_mode):
    ...
    if data["videofilter_keyword"]:
        resp = videofilter_keyword(data, fos, check_mode)
    else:
