...
def router_prefix_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "prefix-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "prefix-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_prefix_list"] = filtered_data
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_prefix_list"]:
        resp = router_prefix_list(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("router_prefix_list"))
    ...
    "children": {
        ...
        "rule": {
            ...
            "children": {
                ...
                "ge": {"v_range": [["v6.0.0", ""]], "type": "integer"},
                "le": {"v_range": [["v6.0.0", ""]], "type": "integer"},
                "flags": {"v_range": [["v6.0.0", "v6.4.4"]], "type": "integer"},
            },
            "v_range": [["v6.0.0", ""]],
        },
    },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_prefix_list"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["router_prefix_list"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_prefix_list"
        )
