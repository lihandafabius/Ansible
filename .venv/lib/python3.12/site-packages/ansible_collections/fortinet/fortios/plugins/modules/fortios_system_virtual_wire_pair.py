...
def system_virtual_wire_pair(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_virtual_wire_pair"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_virtual_wire_pair"]:
        resp = system_virtual_wire_pair(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_virtual_wire_pair")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_virtual_wire_pair"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_virtual_wire_pair"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_virtual_wire_pair"
        )
