...
DOCUMENTATION = """
---
module: fortios_wireless_controller_rf_analysis
short_description: Wireless controller rf-analysis in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify wireless_controller feature and rf_analysis category.
...
def filter_wireless_controller_rf_analysis_data(json):
    option_list = ["wtp_id"]
...
def wireless_controller_rf_analysis(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "rf-analysis", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_rf_analysis"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_rf_analysis"]:
        resp = wireless_controller_rf_analysis(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_rf_analysis")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_rf_analysis"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_rf_analysis"
        )
