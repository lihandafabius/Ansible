...
def filter_firewall_wildcard_fqdn_group_data(json):
    option_list = ["color", "comment", "member", "name", "uuid", "visibility"]
...
def firewall_wildcard_fqdn_group(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_wildcard_fqdn_group"] = filtered_data
...
def fortios_firewall_wildcard_fqdn(data, fos, check_mode):
    ...
    if data["firewall_wildcard_fqdn_group"]:
        resp = firewall_wildcard_fqdn_group(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_wildcard_fqdn_group")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_wildcard_fqdn_group"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_wildcard_fqdn_group"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_wildcard_fqdn_group"
        )

        is_error, has_changed, result, diff = fortios_firewall_wildcard_fqdn(
