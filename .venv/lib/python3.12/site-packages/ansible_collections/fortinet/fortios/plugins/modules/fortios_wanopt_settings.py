...
def wanopt_settings(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("wanopt", "settings", filtered_data, vdom=vdom)
        current_data = fos.get(
            "wanopt", "settings", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["wanopt_settings"] = filtered_data
    ...
    return fos.set(
        "wanopt", "settings", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_wanopt(data, fos, check_mode):
    ...
    if data["wanopt_settings"]:
        resp = wanopt_settings(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("wanopt_settings"))
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "host_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "tunnel_ssl_algorithm": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
        ...
        "auto_detect_algorithm": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "simple"}, {"value": "diff-req-resp"}],
        },
        "tunnel_optimization": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wanopt_settings"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["wanopt_settings"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wanopt_settings"
        )
