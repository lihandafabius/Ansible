...
DOCUMENTATION = """
---
module: fortios_ztna_reverse_connector
short_description: Configure ZTNA Reverse-Connector in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify ztna feature and reverse_connector category.
...
def filter_ztna_reverse_connector_data(json):
    option_list = [
        "address",
        "certificate",
        "default_incoming_vip",
...
def ztna_reverse_connector(data, fos):
    state = None
    vdom = data["vdom"]
    parameters = None
    state = data.get("state", None)
    ztna_reverse_connector_data = data["ztna_reverse_connector"]

    filtered_data = filter_ztna_reverse_connector_data(ztna_reverse_connector_data)
    converted_data = underscore_to_hyphen(filtered_data)

    # pass post processed data to member operations
    # no need to do underscore_to_hyphen since do_member_operation handles it by itself
    data_copy = data.copy()
    data_copy["ztna_reverse_connector"] = filtered_data
...
def fortios_ztna(data, fos):
    ...
    if data["ztna_reverse_connector"]:
        resp = ztna_reverse_connector(data, fos)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("ztna_reverse_connector"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["ztna_reverse_connector"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["ztna_reverse_connector"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "ztna_reverse_connector"
        )
