...
DOCUMENTATION = """
---
module: fortios_cifs_domain_controller
short_description: Define known domain controller servers in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify cifs feature and domain_controller category.
...
def filter_cifs_domain_controller_data(json):
    option_list = [
        "domain_name",
        "ip",
        "ip6",
...
def cifs_domain_controller(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("cifs", "domain-controller", filtered_data, vdom=vdom)
        current_data = fos.get(
            "cifs", "domain-controller", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["cifs_domain_controller"] = filtered_data
    ...
    if state == "present" or state is True:
        ...
        return fos.delete(
            "cifs",
            "domain-controller",
            mkey=converted_data["server-name"],
            vdom=vdom,
            parameters=parameters,
        )
    else:
...
def fortios_cifs(data, fos, check_mode):
    ...
    if data["cifs_domain_controller"]:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["cifs_domain_controller"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["cifs_domain_controller"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "cifs_domain_controller"
        )
