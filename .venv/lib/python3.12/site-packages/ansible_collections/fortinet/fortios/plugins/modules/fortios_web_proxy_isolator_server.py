...
DOCUMENTATION = """
---
module: fortios_web_proxy_isolator_server
short_description: Configure forward-server addresses in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify web_proxy feature and isolator_server category.
...
- name: Configure forward-server addresses.
  fortinet.fortios.fortios_web_proxy_isolator_server:
      vdom: "{{ vdom }}"
      state: "present"
...
def filter_web_proxy_isolator_server_data(json):
    option_list = [
        "addr_type",
        "comment",
        "fabric_force_sync",
...
def web_proxy_isolator_server(data, fos):
    ...
    filtered_data = filter_web_proxy_isolator_server_data(
        web_proxy_isolator_server_data
    )
    ...
    data_copy = data.copy()
    data_copy["web_proxy_isolator_server"] = filtered_data
...
def fortios_web_proxy(data, fos):
    ...
    if data["web_proxy_isolator_server"]:
        resp = web_proxy_isolator_server(data, fos)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("web_proxy_isolator_server")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "addr_type": {
            "v_range": [["v7.6.1", ""]],
            "type": "string",
            "options": [{"value": "ip"}, {"value": "ipv6"}, {"value": "fqdn"}],
        },
        "ip": {"v_range": [["v7.6.1", ""]], "type": "string"},
        "ipv6": {"v_range": [["v7.6.1", ""]], "type": "string"},
        "fqdn": {"v_range": [["v7.6.1", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_isolator_server"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["web_proxy_isolator_server"]["options"][attribute_name][
                "required"
            ] = True
