...
short_description: Configure Websense Integrated Services Protocol (WISP) servers in Fortinet's FortiOS and FortiGate.
description:
...
def web_proxy_wisp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("web-proxy", "wisp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "web-proxy", "wisp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["web_proxy_wisp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "web-proxy", "wisp", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    if data["web_proxy_wisp"]:
        resp = web_proxy_wisp(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("web_proxy_wisp"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_wisp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["web_proxy_wisp"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "web_proxy_wisp"
        )
