...
def user_security_exempt_list(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["user_security_exempt_list"] = filtered_data
...
def fortios_user(data, fos, check_mode):
    ...
    if data["user_security_exempt_list"]:
        resp = user_security_exempt_list(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("user_security_exempt_list")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["user_security_exempt_list"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["user_security_exempt_list"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "user_security_exempt_list"
        )
