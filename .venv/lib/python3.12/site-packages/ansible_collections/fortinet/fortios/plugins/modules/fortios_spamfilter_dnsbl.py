...
def spamfilter_dnsbl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("spamfilter", "dnsbl", filtered_data, vdom=vdom)
        current_data = fos.get(
            "spamfilter", "dnsbl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["spamfilter_dnsbl"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "spamfilter", "dnsbl", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["spamfilter_dnsbl"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["spamfilter_dnsbl"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "spamfilter_dnsbl"
        )
