...
def firewall_pfcp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "pfcp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "pfcp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_pfcp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "firewall", "pfcp", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_pfcp"]:
        resp = firewall_pfcp(data, fos, check_mode)
    else:
