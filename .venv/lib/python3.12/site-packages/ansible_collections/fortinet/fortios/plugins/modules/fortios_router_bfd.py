...
def router_bfd(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "bfd", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "bfd", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_bfd"] = filtered_data
    ...
    return fos.set(
        "router", "bfd", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_bfd"]:
        resp = router_bfd(data, fos, check_mode)
    else:
