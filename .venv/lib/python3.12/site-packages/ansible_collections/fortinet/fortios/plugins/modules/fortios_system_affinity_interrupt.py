...
options:
    ...
    system_affinity_interrupt:
        ...
        suboptions:
            ...
            id:
                description:
                    - ID of the interrupt affinity setting. see <a href='#notes'>Notes</a>.
                required: true
                type: int
            interrupt:
                description:
...
def system_affinity_interrupt(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "affinity-interrupt", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "affinity-interrupt", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_affinity_interrupt"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_affinity_interrupt"]:
        resp = system_affinity_interrupt(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_affinity_interrupt")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_affinity_interrupt"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_affinity_interrupt"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_affinity_interrupt"
        )
