...
options:
    ...
    firewall_ldb_monitor:
        ...
        suboptions:
            ...
            http_get:
                description:
                    - Request URI used to send a GET request to check the health of an HTTP server. Optionally provide a hostname before the first "/" and it
                       will be used as the HTTP Host Header.
                type: str
            http_match:
                description:
...
def firewall_ldb_monitor(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "ldb-monitor", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "ldb-monitor", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ldb_monitor"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_ldb_monitor"]:
        resp = firewall_ldb_monitor(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_ldb_monitor"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "src_ip": {"v_range": [["v6.4.0", ""]], "type": "string"},
        "http_get": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "http_match": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "http_max_redirects": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "dns_protocol": {
            "v_range": [["v7.0.0", ""]],
            "type": "string",
            "options": [{"value": "udp"}, {"value": "tcp"}],
        },
        "dns_request_domain": {"v_range": [["v7.0.0", ""]], "type": "string"},
        "dns_match_ip": {"v_range": [["v7.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ldb_monitor"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ldb_monitor"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ldb_monitor"
        )
