...
def wireless_controller_vap_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "vap-group", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_vap_group"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_vap_group"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_vap_group")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_vap_group"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_vap_group"
        )
