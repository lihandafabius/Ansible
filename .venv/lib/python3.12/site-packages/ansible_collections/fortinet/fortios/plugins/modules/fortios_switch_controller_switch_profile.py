...
def switch_controller_switch_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "switch-controller", "switch-profile", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["switch_controller_switch_profile"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_switch_profile"]:
        resp = switch_controller_switch_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_switch_profile")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_switch_profile"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_switch_profile"
        )
