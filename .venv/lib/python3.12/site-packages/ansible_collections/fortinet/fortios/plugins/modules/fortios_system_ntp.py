...
def system_ntp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "ntp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "ntp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ntp"] = filtered_data
    ...
    return fos.set(
        "system", "ntp", data=converted_data, vdom=vdom, parameters=parameters
    )
