...
def dnsfilter_domain_filter(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["dnsfilter_domain_filter"] = filtered_data
...
def fortios_dnsfilter(data, fos, check_mode):
    ...
    if data["dnsfilter_domain_filter"]:
        resp = dnsfilter_domain_filter(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("dnsfilter_domain_filter"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dnsfilter_domain_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["dnsfilter_domain_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "dnsfilter_domain_filter"
        )
