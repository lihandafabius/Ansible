...
DOCUMENTATION = """
---
module: fortios_antivirus_mms_checksum
short_description: Configure MMS content checksum list in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify antivirus feature and mms_checksum category.
...
options:
    ...
    antivirus_mms_checksum:
        ...
        suboptions:
            ...
            entries:
                description:
                    - modify this MMS content checksum list
                type: list
                elements: dict
                suboptions:
                    checksum:
                        description:
                            - MMS attachment checksum value (CRC32)
                        type: str
                    name:
                        description:
...
def filter_antivirus_mms_checksum_data(json):
...
def antivirus_mms_checksum(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["antivirus_mms_checksum"] = filtered_data
...
def fortios_antivirus(data, fos, check_mode):
    ...
    if data["antivirus_mms_checksum"]:
        resp = antivirus_mms_checksum(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("antivirus_mms_checksum"))
    ...
    "children": {
        ...
        "entries": {
            ...
            "children": {
                ...
                "checksum": {"v_range": [["v6.0.0", "v6.2.7"]], "type": "string"},
                "status": {
                    "v_range": [["v6.0.0", "v6.2.7"]],
                    "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["antivirus_mms_checksum"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["antivirus_mms_checksum"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "antivirus_mms_checksum"
        )
