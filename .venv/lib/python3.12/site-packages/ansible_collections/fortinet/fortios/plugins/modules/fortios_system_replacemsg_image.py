...
def system_replacemsg_image(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "replacemsg-image", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "replacemsg-image", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_replacemsg_image"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_replacemsg_image"]:
        resp = system_replacemsg_image(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "image_base64": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_replacemsg_image"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_image"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_replacemsg_image"
        )
