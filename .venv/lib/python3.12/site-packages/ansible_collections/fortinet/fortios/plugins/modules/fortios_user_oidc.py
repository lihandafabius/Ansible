...
DOCUMENTATION = """
---
module: fortios_user_oidc
short_description: OpenID Connect server entry configuration in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify user feature and oidc category.
...
def filter_user_oidc_data(json):
    option_list = [
        "authorization_url",
        "client_id",
        "client_secret",
...
def user_oidc(data, fos):
    state = None
    vdom = data["vdom"]
    parameters = None
    state = data.get("state", None)
    user_oidc_data = data["user_oidc"]

    filtered_data = filter_user_oidc_data(user_oidc_data)
    converted_data = underscore_to_hyphen(filtered_data)

    # pass post processed data to member operations
    # no need to do underscore_to_hyphen since do_member_operation handles it by itself
    data_copy = data.copy()
    data_copy["user_oidc"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "user", "oidc", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_user(data, fos):
    ...
    if data["user_oidc"]:
        resp = user_oidc(data, fos)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("user_oidc"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "type": {
            "v_range": [["v8.0.0", ""]],
            "type": "string",
            "options": [{"value": "discovery"}, {"value": "manual"}],
        },
        "client_id": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "client_secret": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "discovery_url": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "authorization_url": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "token_url": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "jwks_uri": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "domain_hint": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "issuer": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "verify_issuer": {
            "v_range": [["v8.0.0", ""]],
            "type": "string",
        ...
        "user_regex": {"v_range": [["v8.0.0", ""]], "type": "string"},
        "ldap_server": {
            "type": "list",
            "elements": "dict",
            "children": {
        ...
        "clock_tolerance": {"v_range": [["v8.0.0", ""]], "type": "integer"},
    },
    "v_range": [["v8.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["user_oidc"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["user_oidc"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "user_oidc"
        )
