...
DOCUMENTATION = """
---
module: fortios_sctp_filter_profile
short_description: Configure SCTP filter profiles in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify sctp_filter feature and profile category.
...
def sctp_filter_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("sctp-filter", "profile", filtered_data, vdom=vdom)
        current_data = fos.get(
            "sctp-filter", "profile", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["sctp_filter_profile"] = filtered_data
...
def fortios_sctp_filter(data, fos, check_mode):

    if data["sctp_filter_profile"]:
        resp = sctp_filter_profile(data, fos, check_mode)
    else:
    ...
    "children": {
        ...
        "ppid_filters": {
            ...
            "children": {
                ...
                "ppid": {"v_range": [["v7.0.1", ""]], "type": "integer"},
                "action": {
                    "v_range": [["v7.0.1", ""]],
                    "type": "string",
                    "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["sctp_filter_profile"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["sctp_filter_profile"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "sctp_filter_profile"
        )

        is_error, has_changed, result, diff = fortios_sctp_filter(
