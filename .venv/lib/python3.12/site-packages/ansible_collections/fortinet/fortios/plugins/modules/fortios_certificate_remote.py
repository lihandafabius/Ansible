...
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify certificate feature and remote category.
...
def filter_certificate_remote_data(json):
    option_list = ["name", "range", "remote", "source"]
...
def certificate_remote(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("certificate", "remote", filtered_data, vdom=vdom)
        current_data = fos.get(
            "certificate", "remote", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["certificate_remote"] = filtered_data
...
versioned_schema = {
    ...
    "children": {
        ...
        "remote": {"v_range": [["v6.2.0", ""]], "type": "string"},
        "range": {
            "v_range": [["v6.2.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["certificate_remote"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["certificate_remote"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "certificate_remote"
        )
