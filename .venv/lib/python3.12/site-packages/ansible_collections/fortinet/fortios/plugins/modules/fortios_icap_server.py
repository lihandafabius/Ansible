...
options:
    ...
    icap_server:
        ...
        suboptions:
            ...
            healthcheck:
                description:
                    - Enable/disable ICAP remote server health checking. Attempts to connect to the remote ICAP server to verify that the server is operating
                       normally.
                type: str
                choices:
                    - 'disable'
...
def icap_server(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("icap", "server", filtered_data, vdom=vdom)
        current_data = fos.get(
            "icap", "server", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["icap_server"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "icap", "server", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_icap(data, fos, check_mode):
    ...
    if data["icap_server"]:
        resp = icap_server(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("icap_server"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "ssl_cert": {"v_range": [["v7.0.0", ""]], "type": "string"},
        "healthcheck": {
            "v_range": [["v7.2.0", ""]],
            "type": "string",
            "options": [{"value": "disable"}, {"value": "enable"}],
        },
        "healthcheck_service": {"v_range": [["v7.2.0", ""]], "type": "string"},
        "ip_version": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["icap_server"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["icap_server"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "icap_server"
        )
