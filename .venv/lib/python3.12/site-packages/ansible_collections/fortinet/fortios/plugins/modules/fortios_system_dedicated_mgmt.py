...
def system_dedicated_mgmt(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "dedicated-mgmt", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "dedicated-mgmt", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_dedicated_mgmt"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_dedicated_mgmt"]:
        resp = system_dedicated_mgmt(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "dhcp_netmask": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "dhcp_start_ip": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "dhcp_end_ip": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
}
