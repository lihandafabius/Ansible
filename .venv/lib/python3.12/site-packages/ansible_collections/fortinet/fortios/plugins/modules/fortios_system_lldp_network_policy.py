...
def system_lldp_network_policy(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_lldp_network_policy"] = filtered_data
...
def fortios_system_lldp(data, fos, check_mode):

    if data["system_lldp_network_policy"]:
        resp = system_lldp_network_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_lldp_network_policy")
        )
    ...
    "children": {
        ...
        "voice": {
            ...
            "children": {
                ...
                "vlan": {"v_range": [["v6.2.0", ""]], "type": "integer"},
                "priority": {"v_range": [["v6.2.0", ""]], "type": "integer"},
                "dscp": {"v_range": [["v6.2.0", ""]], "type": "integer"},
            },
        },
        "voice_signaling": {
            "v_range": [["v6.2.0", ""]],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_lldp_network_policy"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_lldp_network_policy"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_lldp_network_policy"
        )

        is_error, has_changed, result, diff = fortios_system_lldp(
