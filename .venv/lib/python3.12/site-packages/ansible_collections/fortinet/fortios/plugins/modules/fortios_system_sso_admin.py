...
def system_sso_admin(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "system", "sso-admin", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_sso_admin"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "sso-admin", data=converted_data, vdom=vdom, parameters=parameters
        )
