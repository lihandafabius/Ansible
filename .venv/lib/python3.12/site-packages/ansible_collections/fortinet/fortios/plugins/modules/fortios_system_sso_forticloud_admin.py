...
DOCUMENTATION = """
---
module: fortios_system_sso_forticloud_admin
short_description: Configure FortiCloud SSO admin users in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and sso_forticloud_admin category.
...
def system_sso_forticloud_admin(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "sso-forticloud-admin", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["system_sso_forticloud_admin"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_sso_forticloud_admin"]:
        resp = system_sso_forticloud_admin(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_sso_forticloud_admin")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sso_forticloud_admin"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_sso_forticloud_admin"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_sso_forticloud_admin"
        )
