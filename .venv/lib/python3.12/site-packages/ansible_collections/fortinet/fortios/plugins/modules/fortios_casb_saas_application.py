...
DOCUMENTATION = """
---
module: fortios_casb_saas_application
short_description: Configure CASB SaaS application in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify casb feature and saas_application category.
...
def casb_saas_application(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "casb", "saas-application", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["casb_saas_application"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["casb_saas_application"]["options"][attribute_name][
                "required"
            ] = True
