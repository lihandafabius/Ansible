...
def wireless_controller_utm_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "utm-profile", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_utm_profile"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_utm_profile"]:
        resp = wireless_controller_utm_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_utm_profile")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "antivirus_profile": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_utm_profile"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_utm_profile"
        )
