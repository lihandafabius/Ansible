...
def system_stp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "stp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "stp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_stp"] = filtered_data
    ...
    return fos.set(
        "system", "stp", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_stp"]:
        resp = system_stp(data, fos, check_mode)
    else:
