...
DOCUMENTATION = """
---
module: fortios_switch_controller_vlan_policy
short_description: Configure VLAN policy to be applied on the managed FortiSwitch ports through dynamic-port-policy in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller feature and vlan_policy category.
...
options:
    ...
    switch_controller_vlan_policy:
        ...
        suboptions:
            ...
            fortilink:
                description:
                    - FortiLink interface for which this VLAN policy belongs to. Source system.interface.name.
                type: str
            name:
                description:
                    - VLAN policy name.
...
def filter_switch_controller_vlan_policy_data(json):
    option_list = [
        "allowed_vlans",
        "allowed_vlans_all",
        "description",
...
def switch_controller_vlan_policy(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_vlan_policy"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_vlan_policy"]:
        resp = switch_controller_vlan_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_vlan_policy")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_vlan_policy"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_vlan_policy"
        )
