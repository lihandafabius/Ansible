...
def log_fortianalyzer_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.fortianalyzer", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.fortianalyzer", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_fortianalyzer_setting"] = filtered_data
...
def fortios_log_fortianalyzer(data, fos, check_mode):

    if data["log_fortianalyzer_setting"]:
        resp = log_fortianalyzer_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_fortianalyzer_setting")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "hmac_algorithm": {
            ...
            "options": [
                {"value": "sha256"},
                {"value": "sha1", "v_range": [["v6.0.0", "v7.4.0"]]},
            ],
        },
        "enc_algorithm": {
            "v_range": [["v6.0.0", ""]],
        ...
        "conn_timeout": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "monitor_keepalive_period": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "monitor_failure_retry_period": {
            "v_range": [["v6.0.0", ""]],
            "type": "integer",
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_fortianalyzer_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortianalyzer_setting"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_fortianalyzer_setting"
        )

        is_error, has_changed, result, diff = fortios_log_fortianalyzer(
