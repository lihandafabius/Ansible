...
def ips_rule(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "ips", "rule", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["ips_rule"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "ips", "rule", data=converted_data, vdom=vdom, parameters=parameters
        )

    elif state == "absent":
        return fos.delete(
            "ips", "rule", mkey=converted_data["name"], vdom=vdom, parameters=parameters
        )
    else:
