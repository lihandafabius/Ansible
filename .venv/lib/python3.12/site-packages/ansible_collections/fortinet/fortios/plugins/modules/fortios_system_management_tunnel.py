...
def system_management_tunnel(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "management-tunnel", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "management-tunnel", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_management_tunnel"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_management_tunnel"]:
        resp = system_management_tunnel(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_management_tunnel")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_management_tunnel"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_management_tunnel"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_management_tunnel"
        )
