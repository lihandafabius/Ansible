...
options:
    ...
    system_arp_table:
        ...
        suboptions:
            id:
                description:
                    - Unique integer ID of the entry. see <a href='#notes'>Notes</a>.
                required: true
                type: int
            interface:
                description:
...
def filter_system_arp_table_data(json):
    option_list = ["id", "interface", "ip", "mac"]
...
def system_arp_table(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "arp-table", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "arp-table", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_arp_table"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "arp-table", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_arp_table"]:
        resp = system_arp_table(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_arp_table"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_arp_table"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_arp_table"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_arp_table"
        )
