...
def spamfilter_bwl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "spamfilter", "bwl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["spamfilter_bwl"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "spamfilter", "bwl", data=converted_data, vdom=vdom, parameters=parameters
        )
