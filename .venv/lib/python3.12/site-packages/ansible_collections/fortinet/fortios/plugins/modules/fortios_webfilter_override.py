...
def webfilter_override(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["webfilter_override"] = filtered_data
...
def fortios_webfilter(data, fos, check_mode):
    ...
    if data["webfilter_override"]:
        resp = webfilter_override(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("webfilter_override"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "user_group": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "old_profile": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "new_profile": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "ip6": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "expires": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "initiator": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["webfilter_override"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_override"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "webfilter_override"
        )
