...
def webfilter_content_header(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("webfilter", "content-header", filtered_data, vdom=vdom)
        current_data = fos.get(
            "webfilter", "content-header", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["webfilter_content_header"] = filtered_data
...
def fortios_webfilter(data, fos, check_mode):
    ...
    if data["webfilter_content_header"]:
        resp = webfilter_content_header(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("webfilter_content_header")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["webfilter_content_header"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_content_header"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "webfilter_content_header"
        )
