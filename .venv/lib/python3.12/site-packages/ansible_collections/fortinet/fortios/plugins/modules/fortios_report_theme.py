...
def report_theme(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "report", "theme", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["report_theme"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "report", "theme", data=converted_data, vdom=vdom, parameters=parameters
        )
