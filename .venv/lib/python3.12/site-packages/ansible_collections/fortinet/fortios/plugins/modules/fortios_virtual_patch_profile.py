...
short_description: Configure virtual-patch profile in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify virtual_patch feature and profile category.
                ...
                suboptions:
                    ...
                    rule:
                        ...
                        suboptions:
                            id:
                                description:
                                    - Rule IDs. see <a href='#notes'>Notes</a>.
                                required: true
                                type: int
                    status:
                        description:
...
def virtual_patch_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("virtual-patch", "profile", filtered_data, vdom=vdom)
        current_data = fos.get(
            "virtual-patch", "profile", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["virtual_patch_profile"] = filtered_data
...
def fortios_virtual_patch(data, fos, check_mode):

    if data["virtual_patch_profile"]:
        resp = virtual_patch_profile(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["virtual_patch_profile"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["virtual_patch_profile"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "virtual_patch_profile"
        )

        is_error, has_changed, result, diff = fortios_virtual_patch(
