...
def system_probe_response(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_probe_response"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_probe_response"]:
        resp = system_probe_response(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_probe_response"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "http_probe_value": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "ttl_mode": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [
        ...
        "mode": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "none"}, {"value": "http-probe"}, {"value": "twamp"}],
        },
        "security_mode": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_probe_response"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_probe_response"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_probe_response"
        )
