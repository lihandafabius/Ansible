...
DOCUMENTATION = """
---
module: fortios_system_evpn
short_description: Configure EVPN instance in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and evpn category.
...
def system_evpn(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "evpn", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "evpn", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_evpn"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "evpn", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_evpn"]:
        resp = system_evpn(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_evpn"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_evpn"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_evpn"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_evpn"
        )
