...
def firewall_vip46(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "vip46", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "vip46", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_vip46"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "firewall", "vip46", data=converted_data, vdom=vdom, parameters=parameters
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "srcintf_filter": {
            ...
            "children": {
                "interface_name": {
                    "v_range": [["v6.2.7", "v6.2.7"], ["v6.4.4", "v6.4.4"]],
                    "type": "string",
                    "required": True,
                }
            },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_vip46"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_vip46"
        )
