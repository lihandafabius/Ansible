...
def wanopt_peer(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("wanopt", "peer", filtered_data, vdom=vdom)
        current_data = fos.get(
            "wanopt", "peer", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["wanopt_peer"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "wanopt", "peer", data=converted_data, vdom=vdom, parameters=parameters
        )
        ...
        return fos.delete(
            "wanopt",
            "peer",
            mkey=converted_data["peer-host-id"],
            vdom=vdom,
...
def fortios_wanopt(data, fos, check_mode):
    ...
    if data["wanopt_peer"]:
        resp = wanopt_peer(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("wanopt_peer"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wanopt_peer"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["wanopt_peer"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wanopt_peer"
        )
