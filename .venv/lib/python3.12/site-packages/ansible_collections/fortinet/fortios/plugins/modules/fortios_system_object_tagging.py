...
def system_object_tagging(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "object-tagging", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "object-tagging", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_object_tagging"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_object_tagging"]:
        resp = system_object_tagging(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_object_tagging"))
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "category": {"v_range": [["v6.0.0", ""]], "type": "string", "required": True},
        "address": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_object_tagging"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_object_tagging"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_object_tagging"
        )
