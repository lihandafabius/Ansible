...
def system_ipv6_tunnel(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "system", "ipv6-tunnel", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ipv6_tunnel"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_ipv6_tunnel"]:
        resp = system_ipv6_tunnel(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_ipv6_tunnel"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_ipv6_tunnel"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_ipv6_tunnel"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_ipv6_tunnel"
        )
