...
def log_syslogd2_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.syslogd2", "filter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.syslogd2", "filter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_syslogd2_filter"] = filtered_data
    ...
    return fos.set(
        "log.syslogd2", "filter", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_log_syslogd2(data, fos, check_mode):
    ...
    if data["log_syslogd2_filter"]:
        resp = log_syslogd2_filter(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_syslogd2_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_syslogd2_filter"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_syslogd2_filter"
        )
