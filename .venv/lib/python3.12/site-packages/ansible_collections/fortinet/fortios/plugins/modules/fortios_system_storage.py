...
def system_storage(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "storage", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "storage", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_storage"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "storage", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_storage"]:
        resp = system_storage(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_storage"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "media_status": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}, {"value": "fail"}],
        },
        "order": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "partition": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "device": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "size": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "usage": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_storage"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_storage"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_storage"
        )
