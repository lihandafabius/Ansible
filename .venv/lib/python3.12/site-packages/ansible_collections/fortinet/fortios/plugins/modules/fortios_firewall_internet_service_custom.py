...
def firewall_internet_service_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "firewall", "internet-service-custom", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["firewall_internet_service_custom"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_internet_service_custom"]:
        resp = firewall_internet_service_custom(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_internet_service_custom")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "master_service_id": {"v_range": [["v6.0.0", "v6.0.11"]], "type": "integer"},
        "disable_entry": {
            "type": "list",
            "elements": "dict",
            "children": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_internet_service_custom"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_internet_service_custom"
        )
