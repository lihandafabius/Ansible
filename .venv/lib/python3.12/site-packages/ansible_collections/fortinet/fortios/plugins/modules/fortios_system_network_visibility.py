...
def system_network_visibility(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "network-visibility", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "network-visibility", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_network_visibility"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_network_visibility"]:
        resp = system_network_visibility(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_network_visibility")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_network_visibility"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_network_visibility"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_network_visibility"
        )
