...
DOCUMENTATION = """
---
module: fortios_wireless_controller_syslog_profile
short_description: Configure Wireless Termination Points (WTP) system log server profile in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify wireless_controller feature and syslog_profile category.
...
def wireless_controller_syslog_profile(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_syslog_profile"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_syslog_profile"]:
        resp = wireless_controller_syslog_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_syslog_profile")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "comment": {"v_range": [["v7.0.2", ""]], "type": "string"},
        "server_status": {
            "v_range": [["v7.0.2", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_syslog_profile"]["options"][attribute_name] = (
