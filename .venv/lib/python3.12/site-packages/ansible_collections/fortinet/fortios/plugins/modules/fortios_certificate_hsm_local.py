...
def filter_certificate_hsm_local_data(json):
    option_list = [
        "api_version",
        "certificate",
        "comments",
...
def certificate_hsm_local(data, fos):
    state = None
    vdom = data["vdom"]
    parameters = None
    state = data.get("state", None)
    certificate_hsm_local_data = data["certificate_hsm_local"]

    filtered_data = filter_certificate_hsm_local_data(certificate_hsm_local_data)
    converted_data = underscore_to_hyphen(filtered_data)

    # pass post processed data to member operations
    # no need to do underscore_to_hyphen since do_member_operation handles it by itself
    data_copy = data.copy()
    data_copy["certificate_hsm_local"] = filtered_data
...
def fortios_certificate(data, fos):
    ...
    if data["certificate_hsm_local"]:
        resp = certificate_hsm_local(data, fos)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("certificate_hsm_local"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["certificate_hsm_local"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["certificate_hsm_local"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "certificate_hsm_local"
        )

        is_error, has_changed, result, diff = fortios_certificate(module.params, fos)
