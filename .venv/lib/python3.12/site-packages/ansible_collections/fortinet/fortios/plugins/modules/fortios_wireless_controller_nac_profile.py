...
DOCUMENTATION = """
---
module: fortios_wireless_controller_nac_profile
short_description: Configure WiFi network access control (NAC) profiles in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify wireless_controller feature and nac_profile category.
...
def filter_wireless_controller_nac_profile_data(json):
    option_list = ["comment", "name", "onboarding_vlan"]
...
def wireless_controller_nac_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "nac-profile", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_nac_profile"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_nac_profile"]:
        resp = wireless_controller_nac_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_nac_profile")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_nac_profile"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_nac_profile"
        )
