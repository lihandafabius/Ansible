...
def wanopt_remote_storage(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("wanopt", "remote-storage", filtered_data, vdom=vdom)
        current_data = fos.get(
            "wanopt", "remote-storage", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["wanopt_remote_storage"] = filtered_data
...
def fortios_wanopt(data, fos, check_mode):
    ...
    if data["wanopt_remote_storage"]:
        resp = wanopt_remote_storage(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "local_cache_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "remote_cache_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "remote_cache_ip": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wanopt_remote_storage"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wanopt_remote_storage"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wanopt_remote_storage"
        )
