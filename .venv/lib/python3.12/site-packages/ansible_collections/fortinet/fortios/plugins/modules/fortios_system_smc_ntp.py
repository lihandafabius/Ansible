...
def filter_system_smc_ntp_data(json):
    option_list = ["channel", "ntpserver", "ntpsync", "syncinterval"]
...
def system_smc_ntp(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_smc_ntp"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_smc_ntp"]:
        resp = system_smc_ntp(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_smc_ntp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_smc_ntp"
        )
