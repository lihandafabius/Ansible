...
def router_multicast_flow(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "multicast-flow", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "multicast-flow", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_multicast_flow"] = filtered_data
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_multicast_flow"]:
        resp = router_multicast_flow(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("router_multicast_flow"))
    ...
    "children": {
        ...
        "flows": {
            ...
            "children": {
                ...
                "group_addr": {"v_range": [["v6.0.0", ""]], "type": "string"},
                "source_addr": {"v_range": [["v6.0.0", ""]], "type": "string"},
            },
            "v_range": [["v6.0.0", ""]],
        },
    },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_multicast_flow"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["router_multicast_flow"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_multicast_flow"
        )
