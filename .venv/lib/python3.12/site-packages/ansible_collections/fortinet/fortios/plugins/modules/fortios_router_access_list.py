...
def router_access_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "access-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "access-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_access_list"] = filtered_data
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_access_list"]:
        resp = router_access_list(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_access_list"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_access_list"
        )
