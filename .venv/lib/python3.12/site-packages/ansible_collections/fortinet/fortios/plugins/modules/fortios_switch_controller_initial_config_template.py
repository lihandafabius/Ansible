...
def filter_switch_controller_initial_config_template_data(json):
    option_list = ["allowaccess", "auto_ip", "dhcp_server", "ip", "name", "vlanid"]
...
def switch_controller_initial_config_template(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_initial_config_template"] = filtered_data
...
def fortios_switch_controller_initial_config(data, fos, check_mode):

    if data["switch_controller_initial_config_template"]:
        resp = switch_controller_initial_config_template(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_initial_config_template")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "vlanid": {"v_range": [["v6.4.0", ""]], "type": "integer"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_initial_config_template"]["options"][
            attribute_name
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_initial_config_template"
        )
