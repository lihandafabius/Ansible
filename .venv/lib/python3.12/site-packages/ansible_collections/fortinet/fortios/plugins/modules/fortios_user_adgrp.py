...
def user_adgrp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("user", "adgrp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "user", "adgrp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["user_adgrp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "user", "adgrp", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_user(data, fos, check_mode):
    ...
    if data["user_adgrp"]:
        resp = user_adgrp(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("user_adgrp"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["user_adgrp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["user_adgrp"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "user_adgrp"
        )
