...
def firewall_schedule_recurring(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_schedule_recurring"] = filtered_data
...
def fortios_firewall_schedule(data, fos, check_mode):
    ...
    if data["firewall_schedule_recurring"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_schedule_recurring")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_schedule_recurring"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_schedule_recurring"]["options"][attribute_name][
                "required"
            ] = True
