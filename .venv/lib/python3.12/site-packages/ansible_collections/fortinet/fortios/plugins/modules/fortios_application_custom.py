...
def application_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("application", "custom", filtered_data, vdom=vdom)
        current_data = fos.get(
            "application", "custom", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["application_custom"] = filtered_data
...
def fortios_application(data, fos, check_mode):
    ...
    if data["application_custom"]:
        resp = application_custom(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("application_custom"))
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "tag": {"v_range": [["v6.0.0", ""]], "type": "string", "required": True},
        "comment": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "signature": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "category": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "protocol": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "technology": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "behavior": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["application_custom"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["application_custom"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "application_custom"
        )
