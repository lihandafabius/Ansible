...
DOCUMENTATION = """
---
module: fortios_system_isf_queue_profile
short_description: Create a queue profile of switch in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and isf_queue_profile category.
...
def system_isf_queue_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "isf-queue-profile", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "isf-queue-profile", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_isf_queue_profile"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_isf_queue_profile"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_isf_queue_profile"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_isf_queue_profile"
        )
