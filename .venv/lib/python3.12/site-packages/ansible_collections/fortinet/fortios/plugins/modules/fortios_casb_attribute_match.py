...
def casb_attribute_match(data, fos):
    ...
    filtered_data = filter_casb_attribute_match_data(casb_attribute_match_data)
    converted_data = underscore_to_hyphen(filtered_data)

    # pass post processed data to member operations
    # no need to do underscore_to_hyphen since do_member_operation handles it by itself
    data_copy = data.copy()
    data_copy["casb_attribute_match"] = filtered_data
...
def fortios_casb(data, fos):
    ...
    if data["casb_attribute_match"]:
        resp = casb_attribute_match(data, fos)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("casb_attribute_match"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "application": {"v_range": [["v7.6.1", ""]], "type": "string"},
        "match_strategy": {
            "v_range": [["v7.6.1", ""]],
            "type": "string",
            "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["casb_attribute_match"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["casb_attribute_match"]["options"][attribute_name]["required"] = True
