...
def firewall_service_category(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.service", "category", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.service", "category", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_service_category"] = filtered_data
...
def fortios_firewall_service(data, fos, check_mode):
    ...
    if data["firewall_service_category"]:
        resp = firewall_service_category(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_service_category")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_service_category"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_service_category"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_service_category"
        )
