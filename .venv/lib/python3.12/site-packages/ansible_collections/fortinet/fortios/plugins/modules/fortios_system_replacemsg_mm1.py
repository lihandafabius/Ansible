...
DOCUMENTATION = """
---
module: fortios_system_replacemsg_mm1
short_description: Replacement messages in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system_replacemsg feature and mm1 category.
...
def filter_system_replacemsg_mm1_data(json):
    option_list = [
        "add_smil",
        "charset",
        "class",
...
def system_replacemsg_mm1(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system.replacemsg", "mm1", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system.replacemsg", "mm1", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_replacemsg_mm1"] = filtered_data
...
def fortios_system_replacemsg(data, fos, check_mode):
    ...
    if data["system_replacemsg_mm1"]:
        resp = system_replacemsg_mm1(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_replacemsg_mm1"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_replacemsg_mm1"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_mm1"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_replacemsg_mm1"
        )
