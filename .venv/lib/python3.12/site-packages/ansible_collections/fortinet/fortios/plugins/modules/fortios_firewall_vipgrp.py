...
def firewall_vipgrp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "vipgrp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "vipgrp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_vipgrp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "firewall", "vipgrp", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_vipgrp"]:
        resp = firewall_vipgrp(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_vipgrp"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_vipgrp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_vipgrp"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_vipgrp"
        )
