...
DOCUMENTATION = """
---
module: fortios_switch_controller_ptp_profile
short_description: Global PTP profile in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller_ptp feature and profile category.
...
options:
    ...
    switch_controller_ptp_profile:
        ...
        suboptions:
            ...
            domain:
                description:
                    - Configure PTP domain value (0 - 255).
                type: int
            mode:
                description:
                    - Select PTP mode.
...
def filter_switch_controller_ptp_profile_data(json):
    option_list = [
        "description",
        "domain",
        "mode",
...
def switch_controller_ptp_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "switch-controller.ptp", "profile", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["switch_controller_ptp_profile"] = filtered_data
...
def fortios_switch_controller_ptp(data, fos, check_mode):
    ...
    if data["switch_controller_ptp_profile"]:
        resp = switch_controller_ptp_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_ptp_profile")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_ptp_profile"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_ptp_profile"
        )
