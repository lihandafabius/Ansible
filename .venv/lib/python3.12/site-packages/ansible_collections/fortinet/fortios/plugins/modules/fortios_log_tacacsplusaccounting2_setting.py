...
DOCUMENTATION = """
---
module: fortios_log_tacacsplusaccounting2_setting
short_description: Settings for TACACS+ accounting in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify log_tacacsplusaccounting2 feature and setting category.
...
def filter_log_tacacsplusaccounting2_setting_data(json):
    option_list = [
        "interface",
        "interface_select_method",
        "port",
...
def log_tacacsplusaccounting2_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "log.tacacs+accounting2", "setting", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["log_tacacsplusaccounting2_setting"] = filtered_data
...
def fortios_log_tacacsplusaccounting2(data, fos, check_mode):
    ...
    if data["log_tacacsplusaccounting2_setting"]:
        resp = log_tacacsplusaccounting2_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_tacacsplusaccounting2_setting")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_tacacsplusaccounting2_setting"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_tacacsplusaccounting2_setting"
        )
