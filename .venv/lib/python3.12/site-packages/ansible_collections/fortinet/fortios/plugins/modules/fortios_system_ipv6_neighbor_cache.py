...
def filter_system_ipv6_neighbor_cache_data(json):
    option_list = ["id", "interface", "ipv6", "mac"]
...
def system_ipv6_neighbor_cache(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "ipv6-neighbor-cache", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "ipv6-neighbor-cache", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ipv6_neighbor_cache"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_ipv6_neighbor_cache"]:
        resp = system_ipv6_neighbor_cache(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_ipv6_neighbor_cache")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_ipv6_neighbor_cache"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_ipv6_neighbor_cache"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_ipv6_neighbor_cache"
        )
