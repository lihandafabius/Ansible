...
def firewall_ipv6_eh_filter(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_ipv6_eh_filter"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_ipv6_eh_filter"]:
        resp = firewall_ipv6_eh_filter(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ipv6_eh_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ipv6_eh_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ipv6_eh_filter"
        )
