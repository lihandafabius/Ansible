...
DOCUMENTATION = """
---
module: fortios_log_fortianalyzer_cloud_filter
short_description: Filters for FortiAnalyzer Cloud in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify log_fortianalyzer_cloud feature and filter category.
...
def log_fortianalyzer_cloud_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "log.fortianalyzer-cloud", "filter", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["log_fortianalyzer_cloud_filter"] = filtered_data
...
def fortios_log_fortianalyzer_cloud(data, fos, check_mode):
    ...
    if data["log_fortianalyzer_cloud_filter"]:
        resp = log_fortianalyzer_cloud_filter(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_fortianalyzer_cloud_filter")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "gtp": {
            "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v7.6.7"]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
        },
        "debug": {
        ...
        "filter": {
            "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v6.4.4"]],
            "type": "string",
        },
        "filter_type": {
            "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v6.4.4"]],
        ...
        "netscan_discovery": {
            "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v6.0.11"]],
            "type": "string",
        },
        "netscan_vulnerability": {
            "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v6.0.11"]],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_fortianalyzer_cloud_filter"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_fortianalyzer_cloud_filter"
        )
