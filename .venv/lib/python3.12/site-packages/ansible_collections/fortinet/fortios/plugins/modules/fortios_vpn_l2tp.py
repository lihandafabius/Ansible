...
def vpn_l2tp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn", "l2tp", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn", "l2tp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_l2tp"] = filtered_data
    ...
    return fos.set("vpn", "l2tp", data=converted_data, vdom=vdom, parameters=parameters)
...
def fortios_vpn(data, fos, check_mode):
    ...
    if data["vpn_l2tp"]:
        resp = vpn_l2tp(data, fos, check_mode)
    else:
