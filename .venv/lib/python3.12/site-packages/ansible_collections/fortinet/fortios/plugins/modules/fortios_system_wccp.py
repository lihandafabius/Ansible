...
def system_wccp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "system", "wccp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_wccp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "wccp", data=converted_data, vdom=vdom, parameters=parameters
        )
        ...
        return fos.delete(
            "system",
            "wccp",
            mkey=converted_data["service-id"],
            vdom=vdom,
