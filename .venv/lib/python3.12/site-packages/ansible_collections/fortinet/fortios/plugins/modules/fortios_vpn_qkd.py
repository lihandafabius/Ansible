...
def vpn_qkd(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn", "qkd", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn", "qkd", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_qkd"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "vpn", "qkd", data=converted_data, vdom=vdom, parameters=parameters
        )

    elif state == "absent":
        return fos.delete(
            "vpn", "qkd", mkey=converted_data["name"], vdom=vdom, parameters=parameters
        )
    else:
...
def fortios_vpn(data, fos, check_mode):
    ...
    if data["vpn_qkd"]:
        resp = vpn_qkd(data, fos, check_mode)
    else:
