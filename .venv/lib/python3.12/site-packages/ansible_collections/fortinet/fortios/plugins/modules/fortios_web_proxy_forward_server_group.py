...
def web_proxy_forward_server_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "web-proxy", "forward-server-group", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["web_proxy_forward_server_group"] = filtered_data
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    if data["web_proxy_forward_server_group"]:
        resp = web_proxy_forward_server_group(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("web_proxy_forward_server_group")
        )
    ...
    "children": {
        ...
        "ldb_method": {
            ...
            "options": [
                {"value": "weighted"},
                {"value": "least-session"},
                {"value": "active-passive", "v_range": [["v6.2.0", ""]]},
            ],
        },
        "group_down_option": {
            "v_range": [["v6.0.0", ""]],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_forward_server_group"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "web_proxy_forward_server_group"
        )
