...
def firewall_policy46(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "policy46", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "policy46", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_policy46"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_policy46"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_policy46"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_policy46"
        )
