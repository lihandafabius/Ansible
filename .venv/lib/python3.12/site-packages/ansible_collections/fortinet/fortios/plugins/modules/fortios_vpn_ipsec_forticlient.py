...
def filter_vpn_ipsec_forticlient_data(json):
    option_list = ["phase2name", "realm", "status", "usergroupname"]
...
def vpn_ipsec_forticlient(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.ipsec", "forticlient", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn.ipsec", "forticlient", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_ipsec_forticlient"] = filtered_data
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    if data["vpn_ipsec_forticlient"]:
        resp = vpn_ipsec_forticlient(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("vpn_ipsec_forticlient"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "usergroupname": {"v_range": [["v6.0.0", "v7.6.0"]], "type": "string"},
        "phase2name": {"v_range": [["v6.0.0", "v7.6.0"]], "type": "string"},
        "status": {
            "v_range": [["v6.0.0", "v7.6.0"]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_ipsec_forticlient"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ipsec_forticlient"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_ipsec_forticlient"
        )
