...
def filter_wanopt_auth_group_data(json):
    option_list = ["auth_method", "cert", "name", "peer", "peer_accept", "psk"]
...
def wanopt_auth_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("wanopt", "auth-group", filtered_data, vdom=vdom)
        current_data = fos.get(
            "wanopt", "auth-group", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["wanopt_auth_group"] = filtered_data
...
def fortios_wanopt(data, fos, check_mode):
    ...
    if data["wanopt_auth_group"]:
        resp = wanopt_auth_group(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("wanopt_auth_group"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "auth_method": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "cert"}, {"value": "psk"}],
        },
        "psk": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "cert": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "peer_accept": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "any"}, {"value": "defined"}, {"value": "one"}],
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wanopt_auth_group"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["wanopt_auth_group"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wanopt_auth_group"
        )
