...
def switch_controller_auto_config_default(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "switch-controller.auto-config", "default", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["switch_controller_auto_config_default"] = filtered_data
...
def fortios_switch_controller_auto_config(data, fos, check_mode):
    ...
    if data["switch_controller_auto_config_default"]:
        resp = switch_controller_auto_config_default(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_auto_config_default")
        )
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "fgt_policy": {"v_range": [["v6.2.0", ""]], "type": "string"},
        "isl_policy": {"v_range": [["v6.2.0", ""]], "type": "string"},
        "icl_policy": {"v_range": [["v6.2.0", ""]], "type": "string"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_auto_config_default"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_auto_config_default"
        )
