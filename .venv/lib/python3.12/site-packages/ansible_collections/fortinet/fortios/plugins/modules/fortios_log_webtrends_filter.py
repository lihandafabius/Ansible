...
def log_webtrends_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.webtrends", "filter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.webtrends", "filter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_webtrends_filter"] = filtered_data
    ...
    return fos.set(
        "log.webtrends", "filter", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_log_webtrends(data, fos, check_mode):

    if data["log_webtrends_filter"]:
        resp = log_webtrends_filter(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_webtrends_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_webtrends_filter"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_webtrends_filter"
        )

        is_error, has_changed, result, diff = fortios_log_webtrends(
