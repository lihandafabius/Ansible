...
def firewall_addrgrp(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "firewall", "addrgrp", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_addrgrp"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "firewall", "addrgrp", data=converted_data, vdom=vdom, parameters=parameters
        )
