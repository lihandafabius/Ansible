...
DOCUMENTATION = """
---
module: fortios_gtp_ie_white_list
short_description: IE white list in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify gtp feature and ie_white_list category.
...
options:
    ...
    gtp_ie_white_list:
        ...
        suboptions:
            entries:
                description:
                    - Entries of white list (to allow) for unknown or out-of-state IEs.
                type: list
                elements: dict
                suboptions:
                    fos_message:
...
def gtp_ie_white_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("gtp", "ie-white-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "gtp", "ie-white-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["gtp_ie_white_list"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["gtp_ie_white_list"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["gtp_ie_white_list"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "gtp_ie_white_list"
        )
