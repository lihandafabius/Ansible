...
def log_fortianalyzer2_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.fortianalyzer2", "filter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.fortianalyzer2", "filter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_fortianalyzer2_filter"] = filtered_data
...
def fortios_log_fortianalyzer2(data, fos, check_mode):
    ...
    if data["log_fortianalyzer2_filter"]:
        resp = log_fortianalyzer2_filter(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_fortianalyzer2_filter")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_fortianalyzer2_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortianalyzer2_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_fortianalyzer2_filter"
        )
