...
def router_static(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "router", "static", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_static"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "router", "static", data=converted_data, vdom=vdom, parameters=parameters
        )
