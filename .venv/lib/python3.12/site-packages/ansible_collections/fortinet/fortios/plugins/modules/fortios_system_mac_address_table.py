...
def system_mac_address_table(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "mac-address-table", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "mac-address-table", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_mac_address_table"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_mac_address_table"]:
        resp = system_mac_address_table(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_mac_address_table")
        )
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "mac": {"v_range": [["v6.0.0", ""]], "type": "string", "required": True},
        "interface": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "reply_substitute": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_mac_address_table"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_mac_address_table"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_mac_address_table"
        )
