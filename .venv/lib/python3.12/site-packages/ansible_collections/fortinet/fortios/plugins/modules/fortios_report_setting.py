...
def report_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("report", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "report", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["report_setting"] = filtered_data
    ...
    return fos.set(
        "report", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_report(data, fos, check_mode):
    ...
    if data["report_setting"]:
        resp = report_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("report_setting"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "report_source": {
            ...
            "type": "list",
            "options": [
                {"value": "forward-traffic"},
                {"value": "sniffer-traffic"},
                {"value": "local-deny-traffic"},
            ],
            "multiple_values": True,
            "elements": "str",
        },
        "web_browsing_threshold": {"v_range": [["v6.0.0", ""]], "type": "integer"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["report_setting"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["report_setting"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "report_setting"
        )
