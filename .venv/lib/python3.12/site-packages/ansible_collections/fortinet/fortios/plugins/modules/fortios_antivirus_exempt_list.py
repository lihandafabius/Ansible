...
DOCUMENTATION = """
---
module: fortios_antivirus_exempt_list
short_description: Configure a list of hashes to be exempt from AV scanning in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify antivirus feature and exempt_list category.
...
def antivirus_exempt_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("antivirus", "exempt-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "antivirus", "exempt-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["antivirus_exempt_list"] = filtered_data
...
def fortios_antivirus(data, fos, check_mode):
    ...
    if data["antivirus_exempt_list"]:
        resp = antivirus_exempt_list(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("antivirus_exempt_list"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["antivirus_exempt_list"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["antivirus_exempt_list"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "antivirus_exempt_list"
        )
