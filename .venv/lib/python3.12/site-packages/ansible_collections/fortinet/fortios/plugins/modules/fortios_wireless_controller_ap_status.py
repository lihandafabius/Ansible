...
options:
    ...
    wireless_controller_ap_status:
        ...
        suboptions:
            ...
            id:
                description:
                    - AP ID. see <a href='#notes'>Notes</a>.
                required: true
                type: int
            ssid:
                description:
...
def filter_wireless_controller_ap_status_data(json):
    option_list = ["bssid", "id", "ssid", "status"]
...
def wireless_controller_ap_status(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "wireless-controller", "ap-status", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_ap_status"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_ap_status"]:
        resp = wireless_controller_ap_status(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("wireless_controller_ap_status")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "bssid": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["wireless_controller_ap_status"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "wireless_controller_ap_status"
        )
