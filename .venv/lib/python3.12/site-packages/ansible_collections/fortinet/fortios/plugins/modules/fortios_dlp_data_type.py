...
DOCUMENTATION = """
---
module: fortios_dlp_data_type
short_description: Configure predefined data type used by DLP blocking in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify dlp feature and data_type category.
...
options:
    ...
    dlp_data_type:
        ...
        suboptions:
            ...
            look_ahead:
                description:
                    - Number of characters to obtain in advance for verification (1 - 255).
                type: int
            look_back:
                description:
...
def dlp_data_type(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("dlp", "data-type", filtered_data, vdom=vdom)
        current_data = fos.get(
            "dlp", "data-type", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["dlp_data_type"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "dlp", "data-type", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_dlp(data, fos, check_mode):
    ...
    if data["dlp_data_type"]:
        resp = dlp_data_type(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("dlp_data_type"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dlp_data_type"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["dlp_data_type"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "dlp_data_type"
        )
