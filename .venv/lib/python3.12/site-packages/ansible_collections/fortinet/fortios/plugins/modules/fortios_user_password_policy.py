...
def user_password_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("user", "password-policy", filtered_data, vdom=vdom)
        current_data = fos.get(
            "user", "password-policy", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["user_password_policy"] = filtered_data
...
def fortios_user(data, fos, check_mode):
    ...
    if data["user_password_policy"]:
        ...
        fos._module.fail_json(msg="missing task body: %s" % ("user_password_policy"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "minimum_length": {"v_range": [["v7.4.1", ""]], "type": "integer"},
        "min_lower_case_letter": {"v_range": [["v7.4.1", ""]], "type": "integer"},
        "min_upper_case_letter": {"v_range": [["v7.4.1", ""]], "type": "integer"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["user_password_policy"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["user_password_policy"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "user_password_policy"
        )
