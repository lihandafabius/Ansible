...
options:
    ...
    system_autoupdate_push_update:
        ...
        suboptions:
            address:
                description:
                    - IPv4 or IPv6 address used by FortiGuard servers to send push updates to this FortiGate.
                type: str
            override:
                description:
...
def filter_system_autoupdate_push_update_data(json):
    option_list = ["address", "override", "port", "status"]
...
def system_autoupdate_push_update(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_autoupdate_push_update"] = filtered_data
...
def fortios_system_autoupdate(data, fos, check_mode):
    ...
    if data["system_autoupdate_push_update"]:
        resp = system_autoupdate_push_update(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_autoupdate_push_update")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "address": {"v_range": [["v6.0.0", "v6.4.4"]], "type": "string"},
        "port": {"v_range": [["v6.0.0", "v6.4.4"]], "type": "integer"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_autoupdate_push_update"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_autoupdate_push_update"
        )
