...
def icap_server_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("icap", "server-group", filtered_data, vdom=vdom)
        current_data = fos.get(
            "icap", "server-group", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["icap_server_group"] = filtered_data
...
def fortios_icap(data, fos, check_mode):
    ...
    if data["icap_server_group"]:
        resp = icap_server_group(data, fos, check_mode)
    else:
