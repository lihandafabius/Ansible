...
def firewall_shaper_per_ip_shaper(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_shaper_per_ip_shaper"] = filtered_data
...
def fortios_firewall_shaper(data, fos, check_mode):
    ...
    if data["firewall_shaper_per_ip_shaper"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_shaper_per_ip_shaper")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "max_concurrent_session": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "max_concurrent_tcp_session": {"v_range": [["v7.0.0", ""]], "type": "integer"},
        "max_concurrent_udp_session": {"v_range": [["v7.0.0", ""]], "type": "integer"},
        "diffserv_forward": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_shaper_per_ip_shaper"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_shaper_per_ip_shaper"
        )
