...
def firewall_ssh_host_key(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.ssh", "host-key", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.ssh", "host-key", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ssh_host_key"] = filtered_data
...
def fortios_firewall_ssh(data, fos, check_mode):
    ...
    if data["firewall_ssh_host_key"]:
        resp = firewall_ssh_host_key(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_ssh_host_key"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "status": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "trusted"}, {"value": "revoked"}],
        },
        "type": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
        ...
        "nid": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "256"}, {"value": "384"}, {"value": "521"}],
        },
        "usage": {
            "v_range": [["v7.0.1", ""]],
            "type": "string",
            "options": [{"value": "transparent-proxy"}, {"value": "access-proxy"}],
        },
        "ip": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "port": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "hostname": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "public_key": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ssh_host_key"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ssh_host_key"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ssh_host_key"
        )
