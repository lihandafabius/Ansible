...
def filter_system_fsso_polling_data(json):
    option_list = ["auth_password", "authentication", "listening_port", "status"]
...
def system_fsso_polling(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "fsso-polling", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "fsso-polling", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_fsso_polling"] = filtered_data
    ...
    return fos.set(
        "system", "fsso-polling", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_fsso_polling"]:
        resp = system_fsso_polling(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_fsso_polling"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "listening_port": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "authentication": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
        },
        "auth_password": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_fsso_polling"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_fsso_polling"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_fsso_polling"
        )
