...
short_description: Configure global DPDK options in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify dpdk feature and global category.
...
options:
    ...
    dpdk_global:
        ...
        suboptions:
            ...
            interface:
                description:
                    - Physical interfaces that enable DPDK.
                type: list
                elements: dict
                suboptions:
                    interface_name:
...
def dpdk_global(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("dpdk", "global", filtered_data, vdom=vdom)
        current_data = fos.get(
            "dpdk", "global", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["dpdk_global"] = filtered_data
    ...
    return fos.set(
        "dpdk", "global", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_dpdk(data, fos, check_mode):
    ...
    if data["dpdk_global"]:
        resp = dpdk_global(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("dpdk_global"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dpdk_global"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["dpdk_global"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "dpdk_global"
        )
