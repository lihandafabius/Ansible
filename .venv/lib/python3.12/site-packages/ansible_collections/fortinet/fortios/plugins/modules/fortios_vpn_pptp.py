...
def filter_vpn_pptp_data(json):
    option_list = ["eip", "ip_mode", "local_ip", "sip", "status", "usrgrp"]
...
def vpn_pptp(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["vpn_pptp"] = filtered_data
...
def fortios_vpn(data, fos, check_mode):
    ...
    if data["vpn_pptp"]:
        resp = vpn_pptp(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("vpn_pptp"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "local_ip": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    fields = {
        ...
        "vpn_pptp": {"required": False, "type": "dict", "default": None, "options": {}},
    }
    for attribute_name in module_spec["options"]:
        fields["vpn_pptp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_pptp"]["options"][attribute_name]["required"] = True
