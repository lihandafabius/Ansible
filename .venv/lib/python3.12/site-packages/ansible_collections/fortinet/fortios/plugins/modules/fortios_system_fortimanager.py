...
def system_fortimanager(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "fortimanager", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "fortimanager", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_fortimanager"] = filtered_data
    ...
    return fos.set(
        "system", "fortimanager", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_fortimanager"]:
        resp = system_fortimanager(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_fortimanager"))
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "ip": {"v_range": [["v6.0.0", "v7.0.1"]], "type": "string"},
        "vdom": {"v_range": [["v6.0.0", "v7.0.1"]], "type": "string"},
        "ipsec": {
            "v_range": [["v6.0.0", "v7.0.1"]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_fortimanager"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_fortimanager"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_fortimanager"
        )
