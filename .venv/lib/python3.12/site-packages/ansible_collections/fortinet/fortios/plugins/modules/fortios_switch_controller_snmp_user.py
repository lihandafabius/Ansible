...
DOCUMENTATION = """
---
module: fortios_switch_controller_snmp_user
short_description: Configure FortiSwitch SNMP v3 users globally in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller feature and snmp_user category.
...
def filter_switch_controller_snmp_user_data(json):
    option_list = [
        "auth_proto",
        "auth_pwd",
        "name",
...
def switch_controller_snmp_user(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_snmp_user"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_snmp_user"]:
        resp = switch_controller_snmp_user(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_snmp_user")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_snmp_user"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_snmp_user"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_snmp_user"
        )
