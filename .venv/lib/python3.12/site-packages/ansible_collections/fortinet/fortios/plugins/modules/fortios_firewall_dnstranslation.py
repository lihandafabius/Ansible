...
def firewall_dnstranslation(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_dnstranslation"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_dnstranslation"]:
        resp = firewall_dnstranslation(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_dnstranslation"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_dnstranslation"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_dnstranslation"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_dnstranslation"
        )
