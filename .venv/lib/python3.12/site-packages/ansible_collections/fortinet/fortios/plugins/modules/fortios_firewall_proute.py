...
DOCUMENTATION = """
---
module: fortios_firewall_proute
short_description: List policy routing in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify firewall feature and proute category.
...
- name: List policy routing.
  fortinet.fortios.fortios_firewall_proute:
      vdom: "{{ vdom }}"
      firewall_proute:
          policy_route_id: "<your_own_value>"
"""
...
def filter_firewall_proute_data(json):
    option_list = ["policy_route_id"]
...
def firewall_proute(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_proute"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_proute"]:
        resp = firewall_proute(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_proute"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_proute"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_proute"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_proute"
        )
