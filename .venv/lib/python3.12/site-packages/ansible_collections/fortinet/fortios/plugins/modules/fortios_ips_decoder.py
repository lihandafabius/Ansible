...
def ips_decoder(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["ips_decoder"] = filtered_data
...
def fortios_ips(data, fos, check_mode):
    ...
    if data["ips_decoder"]:
        resp = ips_decoder(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("ips_decoder"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["ips_decoder"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["ips_decoder"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "ips_decoder"
        )
