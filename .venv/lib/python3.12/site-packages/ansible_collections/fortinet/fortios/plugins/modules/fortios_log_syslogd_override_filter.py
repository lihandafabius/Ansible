...
def log_syslogd_override_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.syslogd", "override-filter", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["log_syslogd_override_filter"] = filtered_data
...
def fortios_log_syslogd(data, fos, check_mode):
    ...
    if data["log_syslogd_override_filter"]:
        resp = log_syslogd_override_filter(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_syslogd_override_filter")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_syslogd_override_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_syslogd_override_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_syslogd_override_filter"
        )
