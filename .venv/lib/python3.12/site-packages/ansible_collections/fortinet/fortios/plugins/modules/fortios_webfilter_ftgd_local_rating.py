...
def filter_webfilter_ftgd_local_rating_data(json):
    option_list = ["comment", "rating", "status", "url"]
...
def webfilter_ftgd_local_rating(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("webfilter", "ftgd-local-rating", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["webfilter_ftgd_local_rating"] = filtered_data
...
def fortios_webfilter(data, fos, check_mode):
    ...
    if data["webfilter_ftgd_local_rating"]:
        resp = webfilter_ftgd_local_rating(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("webfilter_ftgd_local_rating")
        )
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "url": {"v_range": [["v6.0.0", ""]], "type": "string", "required": True},
        "status": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
        },
        "comment": {"v_range": [["v6.4.4", ""]], "type": "string"},
        "rating": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["webfilter_ftgd_local_rating"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_ftgd_local_rating"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "webfilter_ftgd_local_rating"
        )
