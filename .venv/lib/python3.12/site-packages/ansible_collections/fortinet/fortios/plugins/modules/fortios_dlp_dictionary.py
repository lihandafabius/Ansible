...
def dlp_dictionary(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("dlp", "dictionary", filtered_data, vdom=vdom)
        current_data = fos.get(
            "dlp", "dictionary", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["dlp_dictionary"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "dlp", "dictionary", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_dlp(data, fos, check_mode):
    ...
    if data["dlp_dictionary"]:
        resp = dlp_dictionary(data, fos, check_mode)
    else:
