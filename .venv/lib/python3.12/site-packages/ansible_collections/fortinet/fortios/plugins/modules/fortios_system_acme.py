...
DOCUMENTATION = """
---
module: fortios_system_acme
short_description: Configure ACME client in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and acme category.
...
def system_acme(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "acme", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "acme", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_acme"] = filtered_data
    ...
    return fos.set(
        "system", "acme", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_acme"]:
        resp = system_acme(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_acme"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "source_ip": {"v_range": [["v7.0.6", ""]], "type": "string"},
        "source_ip6": {"v_range": [["v7.0.6", ""]], "type": "string"},
        "accounts": {
            "type": "list",
            "elements": "dict",
            "children": {
                "id": {"v_range": [["v7.0.0", ""]], "type": "string", "required": True},
                "status": {"v_range": [["v7.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_acme"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_acme"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_acme"
        )
