...
def router_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "router", "policy", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_policy"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "router", "policy", data=converted_data, vdom=vdom, parameters=parameters
        )
