...
def router_auth_path(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "auth-path", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "auth-path", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_auth_path"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "router", "auth-path", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_auth_path"]:
        resp = router_auth_path(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("router_auth_path"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_auth_path"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["router_auth_path"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_auth_path"
        )
