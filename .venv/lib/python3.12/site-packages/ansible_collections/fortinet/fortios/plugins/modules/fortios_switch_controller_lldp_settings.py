...
def switch_controller_lldp_settings(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_lldp_settings"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_lldp_settings"]:
        resp = switch_controller_lldp_settings(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_lldp_settings")
        )
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "tx_hold": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "tx_interval": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "fast_start_interval": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "management_interface": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "internal"}, {"value": "mgmt"}],
        },
        "device_detection": {
            "v_range": [["v6.4.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_lldp_settings"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_lldp_settings"
        )
