...
DOCUMENTATION = """
---
module: fortios_extension_controller_fortigate
short_description: FortiGate controller configuration in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify extension_controller feature and fortigate category.
...
def filter_extension_controller_fortigate_data(json):
    option_list = [
        "authorized",
        "description",
        "device_id",
...
def extension_controller_fortigate(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "extension-controller", "fortigate", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["extension_controller_fortigate"] = filtered_data
...
def fortios_extension_controller(data, fos, check_mode):
    ...
    if data["extension_controller_fortigate"]:
        resp = extension_controller_fortigate(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("extension_controller_fortigate")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["extension_controller_fortigate"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "extension_controller_fortigate"
        )
