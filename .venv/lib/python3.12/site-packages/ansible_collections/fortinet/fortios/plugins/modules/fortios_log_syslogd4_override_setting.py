...
DOCUMENTATION = """
---
module: fortios_log_syslogd4_override_setting
short_description: Override settings for remote syslog server in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify log_syslogd4 feature and override_setting category.
...
def log_syslogd4_override_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "log.syslogd4", "override-setting", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["log_syslogd4_override_setting"] = filtered_data
...
def fortios_log_syslogd4(data, fos, check_mode):
    ...
    if data["log_syslogd4_override_setting"]:
        resp = log_syslogd4_override_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_syslogd4_override_setting")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_syslogd4_override_setting"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_syslogd4_override_setting"
        )
