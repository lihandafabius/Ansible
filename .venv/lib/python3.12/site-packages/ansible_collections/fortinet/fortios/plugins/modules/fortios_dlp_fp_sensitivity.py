...
def dlp_fp_sensitivity(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("dlp", "fp-sensitivity", filtered_data, vdom=vdom)
        current_data = fos.get(
            "dlp", "fp-sensitivity", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["dlp_fp_sensitivity"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dlp_fp_sensitivity"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["dlp_fp_sensitivity"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "dlp_fp_sensitivity"
        )
