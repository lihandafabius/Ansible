...
def vpn_certificate_ocsp_server(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.certificate", "ocsp-server", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["vpn_certificate_ocsp_server"] = filtered_data
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    if data["vpn_certificate_ocsp_server"]:
        resp = vpn_certificate_ocsp_server(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("vpn_certificate_ocsp_server")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "secondary_url": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "secondary_cert": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "unavail_action": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "revoke"}, {"value": "ignore"}],
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_certificate_ocsp_server"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_certificate_ocsp_server"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_certificate_ocsp_server"
        )
