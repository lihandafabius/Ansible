...
def log_null_device_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.null-device", "filter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.null-device", "filter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_null_device_filter"] = filtered_data
...
def fortios_log_null_device(data, fos, check_mode):
    ...
    if data["log_null_device_filter"]:
        resp = log_null_device_filter(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("log_null_device_filter"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_null_device_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_null_device_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_null_device_filter"
        )
