...
def system_auto_install(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "auto-install", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "auto-install", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_auto_install"] = filtered_data
    ...
    return fos.set(
        "system", "auto-install", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_auto_install"]:
        resp = system_auto_install(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_auto_install"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "default_config_file": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "default_image_file": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_auto_install"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_auto_install"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_auto_install"
        )
