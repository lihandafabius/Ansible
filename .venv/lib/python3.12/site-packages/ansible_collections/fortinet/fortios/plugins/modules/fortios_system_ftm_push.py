...
options:
    ...
    system_ftm_push:
        ...
        suboptions:
            ...
            server_cert:
                description:
                    - Name of the server certificate to be used for SSL. Source certificate.local.name.
                type: str
            server_ip:
                description:
...
def system_ftm_push(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "ftm-push", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "ftm-push", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ftm_push"] = filtered_data
    ...
    return fos.set(
        "system", "ftm-push", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_ftm_push"]:
        resp = system_ftm_push(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_ftm_push"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_ftm_push"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_ftm_push"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_ftm_push"
        )
