...
def system_ddns(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "system", "ddns", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_ddns"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "ddns", data=converted_data, vdom=vdom, parameters=parameters
        )
