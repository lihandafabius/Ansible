...
def firewall_schedule_onetime(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.schedule", "onetime", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.schedule", "onetime", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_schedule_onetime"] = filtered_data
...
def fortios_firewall_schedule(data, fos, check_mode):
    ...
    if data["firewall_schedule_onetime"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_schedule_onetime")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_schedule_onetime"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_schedule_onetime"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_schedule_onetime"
        )
