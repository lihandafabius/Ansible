...
def cifs_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "cifs", "profile", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["cifs_profile"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "cifs", "profile", data=converted_data, vdom=vdom, parameters=parameters
        )
