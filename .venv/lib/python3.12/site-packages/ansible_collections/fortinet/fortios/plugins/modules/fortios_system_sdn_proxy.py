...
DOCUMENTATION = """
---
module: fortios_system_sdn_proxy
short_description: Configure SDN proxy in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and sdn_proxy category.
...
def filter_system_sdn_proxy_data(json):
    option_list = [
        "fabric_force_sync",
        "fabric_object",
        "fabric_object_source",
...
def system_sdn_proxy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "sdn-proxy", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "sdn-proxy", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_sdn_proxy"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "sdn-proxy", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_sdn_proxy"]:
        resp = system_sdn_proxy(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_sdn_proxy"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sdn_proxy"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_sdn_proxy"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_sdn_proxy"
        )
