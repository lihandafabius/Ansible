...
def log_fortiguard_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.fortiguard", "filter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.fortiguard", "filter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_fortiguard_filter"] = filtered_data
...
def fortios_log_fortiguard(data, fos, check_mode):
    ...
    if data["log_fortiguard_filter"]:
        resp = log_fortiguard_filter(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_fortiguard_filter"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortiguard_filter"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_fortiguard_filter"
        )
