...
def filter_firewall_ip_translation_data(json):
    option_list = ["endip", "map_startip", "startip", "transid", "type"]
...
def firewall_ip_translation(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "ip-translation", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "ip-translation", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ip_translation"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_ip_translation"]:
        resp = firewall_ip_translation(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_ip_translation"))
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "transid": {"v_range": [["v6.0.0", ""]], "type": "integer", "required": True},
        "type": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "SCTP"}],
        ...
        "map_startip": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ip_translation"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ip_translation"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ip_translation"
        )
