...
def vpn_certificate_remote(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["vpn_certificate_remote"] = filtered_data
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    if data["vpn_certificate_remote"]:
        resp = vpn_certificate_remote(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("vpn_certificate_remote"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "remote": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "range": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_certificate_remote"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_certificate_remote"]["options"][attribute_name][
                "required"
            ] = True
