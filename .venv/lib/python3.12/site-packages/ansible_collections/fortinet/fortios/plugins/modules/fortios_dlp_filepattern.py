...
def dlp_filepattern(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "dlp", "filepattern", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "dlp", "filepattern", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_dlp(data, fos, check_mode):
    ...
    if data["dlp_filepattern"]:
        ...
        fos._module.fail_json(msg="missing task body: %s" % ("dlp_filepattern"))
        ...
        "entries": {
            ...
            "children": {
                "filter_type": {
                    "v_range": [["v6.0.0", ""]],
                    "type": "string",
                    "options": [{"value": "pattern"}, {"value": "type"}],
                },
                "pattern": {
                    "v_range": [["v6.0.0", ""]],
                    "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dlp_filepattern"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["dlp_filepattern"]["options"][attribute_name]["required"] = True
