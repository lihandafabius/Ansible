...
def webfilter_content(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("webfilter", "content", filtered_data, vdom=vdom)
        current_data = fos.get(
            "webfilter", "content", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["webfilter_content"] = filtered_data
...
def fortios_webfilter(data, fos, check_mode):
    ...
    if data["webfilter_content"]:
        resp = webfilter_content(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("webfilter_content"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["webfilter_content"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_content"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "webfilter_content"
        )
