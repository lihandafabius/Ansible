...
def vpn_ssl_web_realm(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["vpn_ssl_web_realm"] = filtered_data
...
def fortios_vpn_ssl_web(data, fos, check_mode):
    ...
    if data["vpn_ssl_web_realm"]:
        resp = vpn_ssl_web_realm(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("vpn_ssl_web_realm"))
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "url_path": {"v_range": [["v6.0.0", ""]], "type": "string", "required": True},
        "max_concurrent_user": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "login_page": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "virtual_host": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "virtual_host_only": {
            "v_range": [["v6.4.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
        },
        "virtual_host_server_cert": {"v_range": [["v7.0.2", ""]], "type": "string"},
        "radius_server": {"v_range": [["v6.4.0", ""]], "type": "string"},
        "nas_ip": {"v_range": [["v6.4.0", ""]], "type": "string"},
        "radius_port": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_ssl_web_realm"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ssl_web_realm"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_ssl_web_realm"
        )
