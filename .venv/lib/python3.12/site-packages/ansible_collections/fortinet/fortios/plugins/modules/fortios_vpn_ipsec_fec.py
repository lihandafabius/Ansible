...
DOCUMENTATION = """
---
module: fortios_vpn_ipsec_fec
short_description: Configure Forward Error Correction (FEC) mapping profiles in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify vpn_ipsec feature and fec category.
...
def vpn_ipsec_fec(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.ipsec", "fec", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn.ipsec", "fec", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_ipsec_fec"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "vpn.ipsec", "fec", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    if data["vpn_ipsec_fec"]:
        resp = vpn_ipsec_fec(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("vpn_ipsec_fec"))
    ...
    "children": {
        ...
        "mappings": {
            ...
            "children": {
                ...
                "base": {"v_range": [["v7.0.2", ""]], "type": "integer"},
                "redundant": {"v_range": [["v7.0.2", ""]], "type": "integer"},
                "packet_loss_threshold": {
                    "v_range": [["v7.0.2", ""]],
                    "type": "integer",
                },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_ipsec_fec"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ipsec_fec"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_ipsec_fec"
        )
