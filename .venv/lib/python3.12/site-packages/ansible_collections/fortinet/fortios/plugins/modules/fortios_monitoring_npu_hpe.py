...
options:
    ...
    monitoring_npu_hpe:
        ...
        suboptions:
            interval:
                description:
                    - HPE status check interval (1 - 60 seconds).
                type: int
            multipliers:
                description:
                    - HPE type interval multipliers (12 integers from <1> to <255>) number of continuous event logs.
                type: list
                elements: str
            status:
                description:
...
def filter_monitoring_npu_hpe_data(json):
    option_list = ["interval", "multipliers", "status"]
...
def monitoring_npu_hpe(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["monitoring_npu_hpe"] = filtered_data
...
def fortios_monitoring(data, fos, check_mode):
    ...
    if data["monitoring_npu_hpe"]:
        resp = monitoring_npu_hpe(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("monitoring_npu_hpe"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "interval": {"v_range": [["v7.0.0", ""]], "type": "integer"},
        "multipliers": {
            "v_range": [["v7.0.0", ""]],
            "type": "list",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["monitoring_npu_hpe"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["monitoring_npu_hpe"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "monitoring_npu_hpe"
        )
