...
DOCUMENTATION = """
---
module: fortios_extender_lte_carrier_list
short_description: Display FortiExtender modem carrier list in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify extender feature and lte_carrier_list category.
...
def extender_lte_carrier_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("extender", "lte-carrier-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "extender", "lte-carrier-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["extender_lte_carrier_list"] = filtered_data
...
def fortios_extender(data, fos, check_mode):
    ...
    if data["extender_lte_carrier_list"]:
        resp = extender_lte_carrier_list(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("extender_lte_carrier_list")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["extender_lte_carrier_list"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["extender_lte_carrier_list"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "extender_lte_carrier_list"
        )
