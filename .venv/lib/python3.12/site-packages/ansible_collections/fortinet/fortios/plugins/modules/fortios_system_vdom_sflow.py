...
def system_vdom_sflow(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "vdom-sflow", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "vdom-sflow", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_vdom_sflow"] = filtered_data
    ...
    return fos.set(
        "system", "vdom-sflow", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_vdom_sflow"]:
        resp = system_vdom_sflow(data, fos, check_mode)
    else:
