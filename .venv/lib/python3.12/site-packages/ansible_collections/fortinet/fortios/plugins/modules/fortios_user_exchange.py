...
def user_exchange(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "user", "exchange", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["user_exchange"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "user", "exchange", data=converted_data, vdom=vdom, parameters=parameters
        )
