...
def log_syslogd3_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.syslogd3", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.syslogd3", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_syslogd3_setting"] = filtered_data
    ...
    return fos.set(
        "log.syslogd3", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_log_syslogd3(data, fos, check_mode):
    ...
    if data["log_syslogd3_setting"]:
        resp = log_syslogd3_setting(data, fos, check_mode)
    else:
