...
def vpn_ipsec_phase2_interface(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.ipsec", "phase2-interface", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn.ipsec", "phase2-interface", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_ipsec_phase2_interface"] = filtered_data
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    if data["vpn_ipsec_phase2_interface"]:
        ...
        fos._module.fail_json(
            msg="missing task body: %s" % ("vpn_ipsec_phase2_interface")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_ipsec_phase2_interface"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ipsec_phase2_interface"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_ipsec_phase2_interface"
        )
