...
def firewall_ipmacbinding_table(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.ipmacbinding", "table", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["firewall_ipmacbinding_table"] = filtered_data
...
def fortios_firewall_ipmacbinding(data, fos, check_mode):

    if data["firewall_ipmacbinding_table"]:
        resp = firewall_ipmacbinding_table(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_ipmacbinding_table")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ipmacbinding_table"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ipmacbinding_table"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ipmacbinding_table"
        )

        is_error, has_changed, result, diff = fortios_firewall_ipmacbinding(
