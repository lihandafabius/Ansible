...
def spamfilter_iptrust(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "spamfilter", "iptrust", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["spamfilter_iptrust"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["spamfilter_iptrust"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "spamfilter_iptrust"
        )
