...
def log_fortianalyzer_cloud_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "log.fortianalyzer-cloud", "setting", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["log_fortianalyzer_cloud_setting"] = filtered_data
...
def fortios_log_fortianalyzer_cloud(data, fos, check_mode):
    ...
    if data["log_fortianalyzer_cloud_setting"]:
        resp = log_fortianalyzer_cloud_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_fortianalyzer_cloud_setting")
        )
    ...
    "children": {
        ...
        "hmac_algorithm": {
            ...
            "options": [
                ...
                {
                    "value": "sha1",
                    "v_range": [["v6.0.0", "v6.0.0"], ["v6.0.11", "v7.4.0"]],
                },
            ],
        },
        "enc_algorithm": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_fortianalyzer_cloud_setting"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_fortianalyzer_cloud_setting"
        )
