...
def system_vdom_radius_server(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "vdom-radius-server", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "vdom-radius-server", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_vdom_radius_server"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_vdom_radius_server"]:
        resp = system_vdom_radius_server(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_vdom_radius_server")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "radius_server_vdom": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_vdom_radius_server"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_vdom_radius_server"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_vdom_radius_server"
        )
