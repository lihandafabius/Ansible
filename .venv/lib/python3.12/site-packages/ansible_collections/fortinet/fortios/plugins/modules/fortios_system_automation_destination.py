...
def filter_system_automation_destination_data(json):
    option_list = ["destination", "ha_group_id", "name", "type"]
...
def system_automation_destination(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_automation_destination"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_automation_destination"]:
        resp = system_automation_destination(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_automation_destination")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "type": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "fortigate"}, {"value": "ha-cluster"}],
        },
        "destination": {
            "type": "list",
            "elements": "dict",
        ...
        "ha_group_id": {"v_range": [["v6.0.0", ""]], "type": "integer"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_automation_destination"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_automation_destination"
        )
