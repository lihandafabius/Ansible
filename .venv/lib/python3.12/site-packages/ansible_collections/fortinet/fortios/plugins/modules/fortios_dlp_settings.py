...
def dlp_settings(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "dlp", "settings", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    return fos.set(
        "dlp", "settings", data=converted_data, vdom=vdom, parameters=parameters
    )
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "storage_device": {"v_range": [["v6.0.0", ""]], "type": "string"},
        ...
        "db_mode": {
            ...
            "options": [
                {"value": "stop-adding"},
                {"value": "remove-modified-then-oldest"},
                {"value": "remove-oldest"},
            ],
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["dlp_settings"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
