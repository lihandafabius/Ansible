...
def router_prefix_list6(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "prefix-list6", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "prefix-list6", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_prefix_list6"] = filtered_data
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_prefix_list6"]:
        resp = router_prefix_list6(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["router_prefix_list6"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "router_prefix_list6"
        )
