...
def system_auto_script(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "auto-script", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "auto-script", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_auto_script"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_auto_script"]:
        resp = system_auto_script(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_auto_script"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "repeat": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "start": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "manual"}, {"value": "auto"}],
        },
        "script": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "output_size": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "timeout": {"v_range": [["v6.4.0", ""]], "type": "integer"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_auto_script"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_auto_script"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_auto_script"
        )
