...
DOCUMENTATION = """
---
module: fortios_gtp_apn_shaper
short_description: Global per-APN shaper in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify gtp feature and apn_shaper category.
...
options:
    ...
    gtp_apn_shaper:
        ...
        suboptions:
            ...
            apn:
                description:
                    - APN names to match. Leave empty to match ANY. Source gtp.apn.name.
                type: list
                elements: dict
                suboptions:
                    name:
            ...
            id:
                description:
                    - Shaper ID. see <a href='#notes'>Notes</a>.
                required: true
                type: int
            rate_limit:
                description:
...
def filter_gtp_apn_shaper_data(json):
    option_list = ["action", "apn", "back_off_time", "id", "rate_limit"]
...
def gtp_apn_shaper(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["gtp_apn_shaper"] = filtered_data
...
def fortios_gtp(data, fos, check_mode):
    ...
    if data["gtp_apn_shaper"]:
        ...
        fos._module.fail_json(msg="missing task body: %s" % ("gtp_apn_shaper"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["gtp_apn_shaper"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["gtp_apn_shaper"]["options"][attribute_name]["required"] = True
