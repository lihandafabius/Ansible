...
def filter_webfilter_ips_urlfilter_setting6_data(json):
    option_list = ["device", "distance", "gateway6", "geo_filter"]
...
def webfilter_ips_urlfilter_setting6(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "webfilter", "ips-urlfilter-setting6", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["webfilter_ips_urlfilter_setting6"] = filtered_data
...
def fortios_webfilter(data, fos, check_mode):
    ...
    if data["webfilter_ips_urlfilter_setting6"]:
        resp = webfilter_ips_urlfilter_setting6(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("webfilter_ips_urlfilter_setting6")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "geo_filter": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["webfilter_ips_urlfilter_setting6"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "webfilter_ips_urlfilter_setting6"
        )
