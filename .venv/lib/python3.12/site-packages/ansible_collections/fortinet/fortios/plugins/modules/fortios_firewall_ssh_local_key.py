...
def filter_firewall_ssh_local_key_data(json):
    option_list = ["name", "password", "private_key", "public_key", "source"]
...
def firewall_ssh_local_key(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.ssh", "local-key", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.ssh", "local-key", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ssh_local_key"] = filtered_data
...
def fortios_firewall_ssh(data, fos, check_mode):
    ...
    if data["firewall_ssh_local_key"]:
        resp = firewall_ssh_local_key(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_ssh_local_key"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "source": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "built-in"}, {"value": "user"}],
        },
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ssh_local_key"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ssh_local_key"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ssh_local_key"
        )
