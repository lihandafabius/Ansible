...
options:
    ...
    waf_signature:
        ...
        suboptions:
            ...
            id:
                description:
                    - Signature ID. see <a href='#notes'>Notes</a>.
                required: true
                type: int
"""
EXAMPLES = """
...
def waf_signature(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("waf", "signature", filtered_data, vdom=vdom)
        current_data = fos.get(
            "waf", "signature", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["waf_signature"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "waf", "signature", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_waf(data, fos, check_mode):
    ...
    if data["waf_signature"]:
        resp = waf_signature(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("waf_signature"))
...
versioned_schema = {
    ...
    "elements": "dict",
    "children": {
        "desc": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["waf_signature"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["waf_signature"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "waf_signature"
        )
