...
DOCUMENTATION = """
---
module: fortios_system_sso_fortigate_cloud_admin
short_description: Configure FortiCloud SSO admin users in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify system feature and sso_fortigate_cloud_admin category.
...
def system_sso_fortigate_cloud_admin(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "system", "sso-fortigate-cloud-admin", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["system_sso_fortigate_cloud_admin"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_sso_fortigate_cloud_admin"]:
        resp = system_sso_fortigate_cloud_admin(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_sso_fortigate_cloud_admin")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sso_fortigate_cloud_admin"]["options"][attribute_name] = (
