...
def filter_system_proxy_arp_data(json):
    option_list = ["end_ip", "id", "interface", "ip"]
...
def system_proxy_arp(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_proxy_arp"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_proxy_arp"]:
        resp = system_proxy_arp(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_proxy_arp"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_proxy_arp"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_proxy_arp"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_proxy_arp"
        )
