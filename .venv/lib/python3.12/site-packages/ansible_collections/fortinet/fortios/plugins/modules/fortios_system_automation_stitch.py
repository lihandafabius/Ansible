...
def system_automation_stitch(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "automation-stitch", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "automation-stitch", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_automation_stitch"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_automation_stitch"]:
        resp = system_automation_stitch(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_automation_stitch")
        )
    ...
    "children": {
        ...
        "actions": {
            ...
            "children": {
                ...
                "action": {"v_range": [["v7.0.1", ""]], "type": "string"},
                "delay": {"v_range": [["v7.0.1", ""]], "type": "integer"},
                "required": {
                    "v_range": [["v7.0.1", ""]],
                    "type": "string",
                    "options": [{"value": "enable"}, {"value": "disable"}],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_automation_stitch"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_automation_stitch"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_automation_stitch"
        )
