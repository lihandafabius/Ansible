...
def firewall_vipgrp46(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "vipgrp46", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "vipgrp46", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_vipgrp46"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_vipgrp46"]:
        resp = firewall_vipgrp46(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_vipgrp46"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_vipgrp46"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_vipgrp46"
        )
