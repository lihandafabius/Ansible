...
def system_sms_server(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_sms_server"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_sms_server"]:
        resp = system_sms_server(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("system_sms_server"))
...
versioned_schema = {
    ...
    "children": {
        ...
        "mail_server": {"v_range": [["v6.0.0", ""]], "type": "string"},
    },
    "v_range": [["v6.0.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_sms_server"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["system_sms_server"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_sms_server"
        )
