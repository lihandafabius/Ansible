...
def log_eventfilter(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log", "eventfilter", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log", "eventfilter", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_eventfilter"] = filtered_data
    ...
    return fos.set(
        "log", "eventfilter", data=converted_data, vdom=vdom, parameters=parameters
    )
