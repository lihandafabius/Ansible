...
def system_3g_modem_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system.3g-modem", "custom", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system.3g-modem", "custom", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_3g_modem_custom"] = filtered_data
...
def fortios_system_3g_modem(data, fos, check_mode):

    if data["system_3g_modem_custom"]:
        resp = system_3g_modem_custom(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "vendor": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "model": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "vendor_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "product_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "class_id": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "init_string": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "modeswitch_string": {
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_3g_modem_custom"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_3g_modem_custom"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_3g_modem_custom"
        )

        is_error, has_changed, result, diff = fortios_system_3g_modem(
