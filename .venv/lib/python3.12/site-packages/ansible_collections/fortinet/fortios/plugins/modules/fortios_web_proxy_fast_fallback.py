...
DOCUMENTATION = """
---
module: fortios_web_proxy_fast_fallback
short_description: Proxy destination connection fast-fallback in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify web_proxy feature and fast_fallback category.
...
options:
    ...
    web_proxy_fast_fallback:
        ...
        suboptions:
            ...
            connection_timeout:
                description:
                    - Number of milliseconds to wait before starting another connection (200 - 1800000). For sequential connection-mode only.
                type: int
            name:
                description:
...
def web_proxy_fast_fallback(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["web_proxy_fast_fallback"] = filtered_data
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    if data["web_proxy_fast_fallback"]:
        resp = web_proxy_fast_fallback(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("web_proxy_fast_fallback"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_fast_fallback"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["web_proxy_fast_fallback"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "web_proxy_fast_fallback"
        )
