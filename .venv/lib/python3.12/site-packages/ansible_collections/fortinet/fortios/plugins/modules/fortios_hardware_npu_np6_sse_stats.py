...
DOCUMENTATION = """
---
module: fortios_hardware_npu_np6_sse_stats
short_description: Show NP6 hardware session statistics counters in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify hardware_npu_np6 feature and sse_stats category.
...
def hardware_npu_np6_sse_stats(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["hardware_npu_np6_sse_stats"] = filtered_data
...
def fortios_hardware_npu_np6(data, fos, check_mode):
    ...
    if data["hardware_npu_np6_sse_stats"]:
        resp = hardware_npu_np6_sse_stats(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("hardware_npu_np6_sse_stats")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["hardware_npu_np6_sse_stats"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["hardware_npu_np6_sse_stats"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "hardware_npu_np6_sse_stats"
        )
