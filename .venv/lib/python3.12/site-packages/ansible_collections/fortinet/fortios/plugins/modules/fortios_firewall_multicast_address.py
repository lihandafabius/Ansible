...
def firewall_multicast_address(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "multicast-address", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "multicast-address", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_multicast_address"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_multicast_address"]:
        resp = firewall_multicast_address(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("firewall_multicast_address")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "type": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "multicastrange"}, {"value": "broadcastmask"}],
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_multicast_address"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_multicast_address"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_multicast_address"
        )
