...
short_description: Configure web proxy tunneling for the FDN in Fortinet's FortiOS and FortiGate.
description:
...
def filter_system_autoupdate_tunneling_data(json):
    option_list = ["address", "password", "port", "status", "username"]
...
def system_autoupdate_tunneling(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_autoupdate_tunneling"] = filtered_data
...
def fortios_system_autoupdate(data, fos, check_mode):
    ...
    if data["system_autoupdate_tunneling"]:
        resp = system_autoupdate_tunneling(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_autoupdate_tunneling")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_autoupdate_tunneling"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_autoupdate_tunneling"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_autoupdate_tunneling"
        )
