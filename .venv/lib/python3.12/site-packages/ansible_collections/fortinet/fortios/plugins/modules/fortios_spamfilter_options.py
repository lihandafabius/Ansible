...
def spamfilter_options(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("spamfilter", "options", filtered_data, vdom=vdom)
        current_data = fos.get(
            "spamfilter", "options", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["spamfilter_options"] = filtered_data
    ...
    return fos.set(
        "spamfilter", "options", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_spamfilter(data, fos, check_mode):
    ...
    if data["spamfilter_options"]:
        resp = spamfilter_options(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("spamfilter_options"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["spamfilter_options"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["spamfilter_options"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "spamfilter_options"
        )
