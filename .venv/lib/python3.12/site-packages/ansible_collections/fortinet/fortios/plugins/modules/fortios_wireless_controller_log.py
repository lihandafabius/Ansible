...
def wireless_controller_log(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("wireless-controller", "log", filtered_data, vdom=vdom)
        current_data = fos.get(
            "wireless-controller", "log", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["wireless_controller_log"] = filtered_data
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    if data["wireless_controller_log"]:
        resp = wireless_controller_log(data, fos, check_mode)
    else:
