...
def certificate_crl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("certificate", "crl", filtered_data, vdom=vdom)
        current_data = fos.get(
            "certificate", "crl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["certificate_crl"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "certificate", "crl", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_certificate(data, fos, check_mode):
    ...
    if data["certificate_crl"]:
        resp = certificate_crl(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "crl": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "range": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
        ...
        "update_vdom": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "ldap_server": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "ldap_username": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "ldap_password": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "http_url": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "scep_url": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "scep_cert": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["certificate_crl"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
        if mkeyname and mkeyname == attribute_name:
            fields["certificate_crl"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "certificate_crl"
        )
