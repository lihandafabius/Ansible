...
def system_tos_based_priority(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["system_tos_based_priority"] = filtered_data
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_tos_based_priority"]:
        resp = system_tos_based_priority(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("system_tos_based_priority")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "tos": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "priority": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_tos_based_priority"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_tos_based_priority"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_tos_based_priority"
        )
