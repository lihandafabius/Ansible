...
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify emailfilter feature and iptrust category.
...
def filter_emailfilter_iptrust_data(json):
    option_list = [
        "comment",
        "entries",
        "fabric_force_sync",
...
def emailfilter_iptrust(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["emailfilter_iptrust"] = filtered_data
...
def fortios_emailfilter(data, fos, check_mode):
    ...
    if data["emailfilter_iptrust"]:
        resp = emailfilter_iptrust(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("emailfilter_iptrust"))
    ...
    "children": {
        ...
        "entries": {
            ...
            "children": {
                ...
                "ip4_subnet": {"v_range": [["v6.2.0", ""]], "type": "string"},
                "ip6_subnet": {"v_range": [["v6.2.0", ""]], "type": "string"},
            },
            "v_range": [["v6.2.0", ""]],
        },
    },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["emailfilter_iptrust"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["emailfilter_iptrust"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "emailfilter_iptrust"
        )
