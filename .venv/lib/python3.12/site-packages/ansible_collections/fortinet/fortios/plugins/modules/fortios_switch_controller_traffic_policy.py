...
DOCUMENTATION = """
---
module: fortios_switch_controller_traffic_policy
short_description: Configure FortiSwitch traffic policy in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller feature and traffic_policy category.
...
def filter_switch_controller_traffic_policy_data(json):
    option_list = [
        "cos_queue",
        "description",
        "guaranteed_bandwidth",
...
def switch_controller_traffic_policy(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_traffic_policy"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_traffic_policy"]:
        resp = switch_controller_traffic_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_traffic_policy")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "guaranteed_bandwidth": {"v_range": [["v6.2.0", ""]], "type": "integer"},
        "guaranteed_burst": {"v_range": [["v6.2.0", ""]], "type": "integer"},
        "maximum_burst": {"v_range": [["v6.2.0", ""]], "type": "integer"},
        "cos_queue": {"v_range": [["v6.2.0", ""]], "type": "integer"},
        "type": {
            "v_range": [["v6.2.0", "v7.0.5"], ["v7.2.0", "v7.2.0"]],
            "type": "string",
            "options": [{"value": "ingress"}, {"value": "egress"}],
        },
        "id": {"v_range": [["v6.2.3", "v6.2.3"]], "type": "integer"},
    },
    "v_range": [["v6.2.0", ""]],
}
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_traffic_policy"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_traffic_policy"
        )
