...
def firewall_auth_portal(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_auth_portal"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_auth_portal"]:
        resp = firewall_auth_portal(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "portal_addr": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "portal_addr6": {"v_range": [["v6.0.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_auth_portal"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_auth_portal"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_auth_portal"
        )
