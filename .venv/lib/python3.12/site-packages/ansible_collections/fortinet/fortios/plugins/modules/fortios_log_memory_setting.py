...
def log_memory_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.memory", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.memory", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_memory_setting"] = filtered_data
    ...
    return fos.set(
        "log.memory", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_log_memory(data, fos, check_mode):
    ...
    if data["log_memory_setting"]:
        resp = log_memory_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("log_memory_setting"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_memory_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_memory_setting"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_memory_setting"
        )
