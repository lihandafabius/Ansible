...
DOCUMENTATION = """
---
module: fortios_switch_controller_remote_log
short_description: Configure logging by FortiSwitch device to a remote syslog server in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller feature and remote_log category.
...
def filter_switch_controller_remote_log_data(json):
    option_list = ["csv", "facility", "name", "port", "server", "severity", "status"]
...
def switch_controller_remote_log(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("switch-controller", "remote-log", filtered_data, vdom=vdom)
    ...
    data_copy = data.copy()
    data_copy["switch_controller_remote_log"] = filtered_data
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    if data["switch_controller_remote_log"]:
        resp = switch_controller_remote_log(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_remote_log")
        )
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_remote_log"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_remote_log"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_remote_log"
        )
