...
def system_session_ttl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "session-ttl", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "session-ttl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_session_ttl"] = filtered_data
    ...
    return fos.set(
        "system", "session-ttl", data=converted_data, vdom=vdom, parameters=parameters
    )
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "default": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "port": {
            "type": "list",
            "elements": "dict",
            "children": {
                ...
                "timeout": {"v_range": [["v6.0.0", ""]], "type": "string"},
                "refresh_direction": {
                    "v_range": [["v7.4.1", ""]],
                    "type": "string",
                    "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["system_session_ttl"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "system_session_ttl"
        )
