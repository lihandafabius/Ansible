...
def log_custom_field(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["log_custom_field"] = filtered_data
...
def fortios_log(data, fos, check_mode):
    ...
    if data["log_custom_field"]:
        resp = log_custom_field(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_custom_field"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "log_custom_field"
        )
