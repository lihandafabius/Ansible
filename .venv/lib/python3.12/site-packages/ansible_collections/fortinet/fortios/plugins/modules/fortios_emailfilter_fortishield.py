...
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify emailfilter feature and fortishield category.
...
def filter_emailfilter_fortishield_data(json):
...
def emailfilter_fortishield(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("emailfilter", "fortishield", filtered_data, vdom=vdom)
        current_data = fos.get(
            "emailfilter", "fortishield", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["emailfilter_fortishield"] = filtered_data
...
def fortios_emailfilter(data, fos, check_mode):
    ...
    if data["emailfilter_fortishield"]:
        resp = emailfilter_fortishield(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("emailfilter_fortishield"))
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "spam_submit_srv": {"v_range": [["v6.2.0", ""]], "type": "string"},
        "spam_submit_force": {
            "v_range": [["v6.2.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["emailfilter_fortishield"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["emailfilter_fortishield"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "emailfilter_fortishield"
        )
