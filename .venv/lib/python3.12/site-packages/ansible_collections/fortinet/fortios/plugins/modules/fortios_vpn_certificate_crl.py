...
def vpn_certificate_crl(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.certificate", "crl", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn.certificate", "crl", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_certificate_crl"] = filtered_data
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    if data["vpn_certificate_crl"]:
        resp = vpn_certificate_crl(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_certificate_crl"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_certificate_crl"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_certificate_crl"
        )
