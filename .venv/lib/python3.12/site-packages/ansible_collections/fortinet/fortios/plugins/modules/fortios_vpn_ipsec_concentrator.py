...
options:
    ...
    vpn_ipsec_concentrator:
        ...
        suboptions:
            id:
                description:
                    - Concentrator ID (1 - 65535). see <a href='#notes'>Notes</a>.
                required: true
                type: int
            member:
                description:
...
def filter_vpn_ipsec_concentrator_data(json):
    option_list = ["id", "member", "name", "src_check"]
...
def vpn_ipsec_concentrator(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("vpn.ipsec", "concentrator", filtered_data, vdom=vdom)
        current_data = fos.get(
            "vpn.ipsec", "concentrator", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["vpn_ipsec_concentrator"] = filtered_data
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["vpn_ipsec_concentrator"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ipsec_concentrator"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "vpn_ipsec_concentrator"
        )
