...
options:
    ...
    web_proxy_url_match:
        ...
        suboptions:
            ...
            fast_fallback:
                description:
                    - Fast fallback configuration entry name. Source web-proxy.fast-fallback.name.
                type: str
            forward_server:
                description:
...
- name: Exempt URLs from web proxy forwarding, caching and fast-fallback.
  fortinet.fortios.fortios_web_proxy_url_match:
      ...
      web_proxy_url_match:
          ...
          comment: "Comment."
          fast_fallback: "<your_own_value> (source web-proxy.fast-fallback.name)"
...
def web_proxy_url_match(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("web-proxy", "url-match", filtered_data, vdom=vdom)
        current_data = fos.get(
            "web-proxy", "url-match", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["web_proxy_url_match"] = filtered_data
...
versioned_schema = {
    ...
    "children": {
        ...
        "forward_server": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "fast_fallback": {"v_range": [["v7.4.1", ""]], "type": "string"},
        "cache_exemption": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["web_proxy_url_match"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "web_proxy_url_match"
        )
