...
options:
    ...
    certificate_local:
        ...
        suboptions:
            ...
            est_server_cert:
                description:
                    - EST server"s certificate must be verifiable by this certificate to be authenticated. Source certificate.ca.name certificate.remote.name.
                type: str
            est_srp_password:
                description:
...
def certificate_local(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["certificate_local"] = filtered_data
...
def fortios_certificate(data, fos, check_mode):
    ...
    if data["certificate_local"]:
        resp = certificate_local(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("certificate_local"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["certificate_local"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "certificate_local"
        )
