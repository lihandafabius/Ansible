...
def filter_switch_controller_qos_queue_policy_data(json):
    option_list = ["cos_queue", "name", "rate_by", "schedule"]
...
def switch_controller_qos_queue_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey(
            "switch-controller.qos", "queue-policy", filtered_data, vdom=vdom
        )
    ...
    data_copy = data.copy()
    data_copy["switch_controller_qos_queue_policy"] = filtered_data
...
def fortios_switch_controller_qos(data, fos, check_mode):

    if data["switch_controller_qos_queue_policy"]:
        resp = switch_controller_qos_queue_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_qos_queue_policy")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "rate_by": {
            "v_range": [["v6.2.0", ""]],
            "type": "string",
            "options": [{"value": "kbps"}, {"value": "percent"}],
        },
        "cos_queue": {
            "type": "list",
            "elements": "dict",
            "children": {
                ...
                "min_rate": {"v_range": [["v6.0.0", ""]], "type": "integer"},
                "max_rate": {"v_range": [["v6.0.0", ""]], "type": "integer"},
                "min_rate_percent": {"v_range": [["v6.2.0", ""]], "type": "integer"},
                "max_rate_percent": {"v_range": [["v6.2.0", ""]], "type": "integer"},
                "drop_policy": {
                    "v_range": [["v6.0.0", ""]],
                    "type": "string",
                    "options": [
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_qos_queue_policy"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_qos_queue_policy"
        )

        is_error, has_changed, result, diff = fortios_switch_controller_qos(
