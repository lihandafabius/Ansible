...
def spamfilter_fortishield(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("spamfilter", "fortishield", filtered_data, vdom=vdom)
        current_data = fos.get(
            "spamfilter", "fortishield", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["spamfilter_fortishield"] = filtered_data
...
def fortios_spamfilter(data, fos, check_mode):
    ...
    if data["spamfilter_fortishield"]:
        resp = spamfilter_fortishield(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("spamfilter_fortishield"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["spamfilter_fortishield"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["spamfilter_fortishield"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "spamfilter_fortishield"
        )
