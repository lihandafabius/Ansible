...
def firewall_ssh_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.ssh", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.ssh", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ssh_setting"] = filtered_data
    ...
    return fos.set(
        "firewall.ssh", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_firewall_ssh(data, fos, check_mode):
    ...
    if data["firewall_ssh_setting"]:
        resp = firewall_ssh_setting(data, fos, check_mode)
    else:
...
versioned_schema = {
    ...
    "children": {
        ...
        "hostkey_rsa2048": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "hostkey_dsa1024": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "hostkey_ecdsa256": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "hostkey_ecdsa384": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "hostkey_ecdsa521": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "hostkey_ed25519": {"v_range": [["v6.0.0", ""]], "type": "string"},
        "host_trusted_checking": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ssh_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ssh_setting"
        )

        is_error, has_changed, result, diff = fortios_firewall_ssh(
