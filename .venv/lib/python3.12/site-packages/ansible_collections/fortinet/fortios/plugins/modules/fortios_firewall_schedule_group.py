...
- name: Schedule group configuration.
  fortinet.fortios.fortios_firewall_schedule_group:
      ...
      firewall_schedule_group:
          ...
          member:
              -
                  name: "default_name_8 (source firewall.schedule.onetime.name firewall.schedule.recurring.name)"
          name: "default_name_9"
          uuid: "<your_own_value>"
"""
...
def firewall_schedule_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.schedule", "group", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.schedule", "group", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_schedule_group"] = filtered_data
...
def fortios_firewall_schedule(data, fos, check_mode):
    ...
    if data["firewall_schedule_group"]:
        ...
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_schedule_group"))
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_schedule_group"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_schedule_group"]["options"][attribute_name][
                "required"
            ] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_schedule_group"
        )

        is_error, has_changed, result, diff = fortios_firewall_schedule(
