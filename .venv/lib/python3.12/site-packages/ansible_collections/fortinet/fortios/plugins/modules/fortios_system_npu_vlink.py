...
def system_npu_vlink(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("system", "npu-vlink", filtered_data, vdom=vdom)
        current_data = fos.get(
            "system", "npu-vlink", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["system_npu_vlink"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "system", "npu-vlink", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_system(data, fos, check_mode):
    ...
    if data["system_npu_vlink"]:
        resp = system_npu_vlink(data, fos, check_mode)
    else:
