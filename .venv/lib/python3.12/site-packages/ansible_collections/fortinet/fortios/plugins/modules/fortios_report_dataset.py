...
def report_dataset(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "report", "dataset", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["report_dataset"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "report", "dataset", data=converted_data, vdom=vdom, parameters=parameters
        )
