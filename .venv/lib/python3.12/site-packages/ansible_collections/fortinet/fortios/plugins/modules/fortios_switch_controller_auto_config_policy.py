...
DOCUMENTATION = """
---
module: fortios_switch_controller_auto_config_policy
short_description: Policy definitions which can define the behavior on auto configured interfaces in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS (FOS) device by allowing the
      user to set and modify switch_controller_auto_config feature and policy category.
...
options:
    ...
    switch_controller_auto_config_policy:
        ...
        suboptions:
            ...
            qos_policy:
                description:
                    - Auto-Config QoS policy. Source switch-controller.qos.qos-policy.name.
                type: str
...
def filter_switch_controller_auto_config_policy_data(json):
    option_list = [
        "igmp_flood_report",
        "igmp_flood_traffic",
        "name",
...
def switch_controller_auto_config_policy(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["switch_controller_auto_config_policy"] = filtered_data
...
def fortios_switch_controller_auto_config(data, fos, check_mode):

    if data["switch_controller_auto_config_policy"]:
        resp = switch_controller_auto_config_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("switch_controller_auto_config_policy")
        )
...
versioned_schema = {
    ...
    "children": {
        ...
        "qos_policy": {"v_range": [["v6.2.0", ""]], "type": "string"},
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["switch_controller_auto_config_policy"]["options"][attribute_name] = (
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "switch_controller_auto_config_policy"
        )

        is_error, has_changed, result, diff = fortios_switch_controller_auto_config(
