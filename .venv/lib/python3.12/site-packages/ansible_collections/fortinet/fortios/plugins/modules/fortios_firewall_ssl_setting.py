...
def firewall_ssl_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall.ssl", "setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall.ssl", "setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_ssl_setting"] = filtered_data
    ...
    return fos.set(
        "firewall.ssl", "setting", data=converted_data, vdom=vdom, parameters=parameters
    )
...
def fortios_firewall_ssl(data, fos, check_mode):

    if data["firewall_ssl_setting"]:
        resp = firewall_ssl_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(msg="missing task body: %s" % ("firewall_ssl_setting"))
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "proxy_connect_timeout": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "ssl_dh_bits": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [
        ...
        "session_cache_capacity": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "session_cache_timeout": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "kxp_queue_threshold": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "ssl_queue_threshold": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "abbreviate_handshake": {
            "v_range": [["v6.0.0", ""]],
            "type": "string",
            "options": [{"value": "enable"}, {"value": "disable"}],
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_ssl_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ssl_setting"]["options"][attribute_name]["required"] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_ssl_setting"
        )

        is_error, has_changed, result, diff = fortios_firewall_ssl(
