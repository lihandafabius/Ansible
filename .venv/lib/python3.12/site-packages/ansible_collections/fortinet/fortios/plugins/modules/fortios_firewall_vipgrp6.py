...
def firewall_vipgrp6(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("firewall", "vipgrp6", filtered_data, vdom=vdom)
        current_data = fos.get(
            "firewall", "vipgrp6", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["firewall_vipgrp6"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "firewall", "vipgrp6", data=converted_data, vdom=vdom, parameters=parameters
        )
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_vipgrp6"]:
        resp = firewall_vipgrp6(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_vipgrp6"]["options"][attribute_name] = module_spec["options"][
            attribute_name
        ]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_vipgrp6"
        )
