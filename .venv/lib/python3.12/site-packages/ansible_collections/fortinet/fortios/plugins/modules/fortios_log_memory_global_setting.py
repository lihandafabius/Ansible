...
def log_memory_global_setting(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("log.memory", "global-setting", filtered_data, vdom=vdom)
        current_data = fos.get(
            "log.memory", "global-setting", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["log_memory_global_setting"] = filtered_data
...
def fortios_log_memory(data, fos, check_mode):
    ...
    if data["log_memory_global_setting"]:
        resp = log_memory_global_setting(data, fos, check_mode)
    else:
        fos._module.fail_json(
            msg="missing task body: %s" % ("log_memory_global_setting")
        )
...
versioned_schema = {
    ...
    "type": "dict",
    "children": {
        "max_size": {"v_range": [["v6.0.0", ""]], "type": "integer"},
        "full_first_warning_threshold": {
            "v_range": [["v6.0.0", ""]],
            "type": "integer",
        },
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["log_memory_global_setting"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_memory_global_setting"]["options"][attribute_name][
                "required"
            ] = True
