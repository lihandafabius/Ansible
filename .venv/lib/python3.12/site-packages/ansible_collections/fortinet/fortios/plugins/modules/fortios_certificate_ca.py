...
def certificate_ca(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        current_data = fos.get(
            "certificate", "ca", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["certificate_ca"] = filtered_data
    ...
    if state == "present" or state is True:
        return fos.set(
            "certificate", "ca", data=converted_data, vdom=vdom, parameters=parameters
        )
