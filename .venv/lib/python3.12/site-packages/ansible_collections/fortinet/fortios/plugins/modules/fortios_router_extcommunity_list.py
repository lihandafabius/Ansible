...
def router_extcommunity_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey("router", "extcommunity-list", filtered_data, vdom=vdom)
        current_data = fos.get(
            "router", "extcommunity-list", vdom=vdom, mkey=mkey, parameters=parameters
        )
        is_existed = (
            current_data
    ...
    data_copy = data.copy()
    data_copy["router_extcommunity_list"] = filtered_data
...
def fortios_router(data, fos, check_mode):
    ...
    if data["router_extcommunity_list"]:
        resp = router_extcommunity_list(data, fos, check_mode)
    else:
