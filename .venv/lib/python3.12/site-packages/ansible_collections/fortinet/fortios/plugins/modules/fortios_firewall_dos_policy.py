...
def firewall_dos_policy(data, fos, check_mode=False):
    ...
    data_copy = data.copy()
    data_copy["firewall_dos_policy"] = filtered_data
...
def fortios_firewall(data, fos, check_mode):
    ...
    if data["firewall_dos_policy"]:
        resp = firewall_dos_policy(data, fos, check_mode)
    else:
...
def main():
    ...
    for attribute_name in module_spec["options"]:
        fields["firewall_dos_policy"]["options"][attribute_name] = module_spec[
            "options"
        ][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(
            fos, versioned_schema, "firewall_dos_policy"
        )
