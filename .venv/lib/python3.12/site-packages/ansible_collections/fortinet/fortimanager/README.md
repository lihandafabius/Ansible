...
# fortinet.fortimanager:2.15.0 - configuring FortiManager
...
* `fmgr_antivirus_profile_websocket`  Configure WEBSOCKET AntiVirus options.
...
* `fmgr_casb_profile_saasapplication_advancedtenantcontrol`  CASB profile advanced tenant control.
* `fmgr_casb_profile_saasapplication_advancedtenantcontrol_attribute`  CASB advanced tenant control attribute.
...
* `fmgr_casb_saasapplication_inputattributes`  SaaS application input attributes.
* `fmgr_casb_saasapplication_outputattributes`  SaaS application output attributes.
...
* `fmgr_casb_useractivity_match_tenantextraction`  CASB user activity tenant extraction.
* `fmgr_casb_useractivity_match_tenantextraction_filters`  CASB user activity tenant extraction filters.
* `fmgr_casb_useractivity_match_tenantsessionextraction`  CASB user activity tenant session extraction.
* `fmgr_casb_useractivity_match_tenantsessionextraction_filters`  CASB user activity session extraction filters.
...
* `fmgr_deployment_get_controller_status`  Refresh status of AP/Switch/Extender controller.
...
* `fmgr_devprof_system_template_interface`  System template system template interface.
* `fmgr_devprof_system_template_interface_iprange`  System template system template interface ip range.
...
* `fmgr_dynamic_log_npuserver_servergroup`  Dynamic log npu server server group.
...
* `fmgr_extensioncontroller_extenderprofile_lanextension_trafficsplitservices`  Config FortiExtender traffic split interface for LAN extension.
...
* `fmgr_firewall_customtag`  Define custom tag table.
...
* `fmgr_firewall_profileprotocoloptions_websocket`  Configure WebSocket protocol options.
...
* `fmgr_firewall_proxyaddrgrp6_tagging`  Config object tagging.
...
* `fmgr_fmg_script`  Fmg script.
* `fmgr_fmg_script_schedule`  Fmg script schedule.
...
* `fmgr_pm_config_pblock_firewall_localinpolicy`  Configure user defined IPv4 local-in policies.
* `fmgr_pm_config_pblock_firewall_localinpolicy6`  Configure user defined IPv6 local-in policies.
...
* `fmgr_securityconsole_template_validate`  Securityconsole template validate.
...
* `fmgr_switchcontroller_managedswitch_systemdhcpserver_iprange`  DHCP IP range configuration.
...
* `fmgr_switchcontroller_securitypolicy_admin`  Configure fortiswitchs admin security-policy.
...
* `fmgr_sys_backup`  Backup FortiManager configuration.
...
* `fmgr_system_csf_trustedlist_adom`  Cli system csf trusted list adom.
...
* `fmgr_system_externalresource_dynamicmapping`  System external resource dynamic mapping.
...
* `fmgr_system_locallog_tacacsaccounting_filter`  Cli system locallog tacacs+accounting filter.
* `fmgr_system_locallog_tacacsaccounting_setting`  Cli system locallog tacacs+accounting setting.
* `fmgr_system_log_alert`  Log based alert settings.
* `fmgr_system_log_apiratelimit`  Cli system log api ratelimit.
...
* `fmgr_system_log_settings_clientcertauth`  Cli system log settings client cert auth.
...
* `fmgr_system_npu_icmperrorratectrl`  Configure the rate of ICMP errors generated by this FortiGate, which is achieved by token bucket algorithm.
...
* `fmgr_user_aci`  User aci.
* `fmgr_user_adgrp`  Configure FSSO groups.
* `fmgr_user_azure`  User azure.
* `fmgr_user_azure_rule`  User azure rule.
...
* `fmgr_user_guardicore`  User guardicore.
...
* `fmgr_user_local_dynamicmapping`  Configure local users.
...
* `fmgr_vpn_ipsec_fec_mappings_tos`  FEC redundancy mapping table for specific type of service.
...
* `fmgr_vpn_qkd`  Configure Quantum Key Distribution servers.
...
* `fmgr_wanprof_system_sdwan_healthcheckfortiguard`  SD-WAN status checking or health checking.
* `fmgr_wanprof_system_sdwan_healthcheckfortiguard_sla`  Service level agreement.
...
* `fmgr_webfilter_profile_ftgdwf_risk`  FortiGuard risk level settings.
...
* `fmgr_webproxy_isolatorserver`  Configure forward-server addresses.
...
* `fmgr_wireless_lwprofile`  Configure LoRaWAN profile.
...
* `fmgr_ztna_destination`  Configure ZTNA destination.
...
* `fmgr_ztna_trafficforwardproxy_quic`  Ztna traffic forward proxy quic.
* `fmgr_ztna_trafficforwardproxy_sslciphersuites`  Ztna traffic forward proxy ssl cipher suites.
...
* `fmgr_ztna_webproxy_apigateway6_quic`  QUIC setting.
...
* `fmgr_ztna_webproxy_apigateway_quic`  QUIC setting.
* `fmgr_ztna_webproxy_apigateway_realservers`  Select the real servers that this Access Proxy will distribute traffic to.
* `fmgr_ztna_webproxy_apigateway_sslciphersuites`  SSL/TLS cipher suites to offer to a server, ordered by priority.
